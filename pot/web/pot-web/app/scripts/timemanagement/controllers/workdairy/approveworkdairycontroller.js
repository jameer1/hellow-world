...
app.config(["$stateProvider", function($stateProvider) {
	$stateProvider.state("approveworkdiary", {
		...
		views : {
			'content@' : {
				templateUrl : 'views/timemanagement/workdairy/approveworkdairy/workdairyapproveworkdairy.html',
		...
		["$scope", "$state","$q", "ngDialog", "blockUI", "WageFactory", "WorkDairyApproveFactory", "POScheduleItemsFactory", "PlantRegisterService", "WorkDairyDocketFactory", "WorkDairyProgressFactory", "EpsProjectSelectFactory", "WorkDairyCopyFactory", "UserService", "$rootScope", "WorkDairySubmitFactory", "WorkDairyEmpFactory", "WorkDairyPlantFactory", "MaterialFactory", "ProjectCrewPopupService", "CreateWorkShiftPopupFactory", "WorkDiaryService", "ModuleUserFactory", "WeatherService", "CreateWeatherPopupFactory", "WorkDairyCostCodeFactory", "RegisterPurchaseOrderFactory", "WorkDairyMaterialProjDocketFactory", "GenericAlertService", "WorkDairyAddProgressFactory", "RequestForAdditionalTimeFactory", "MaterialRegisterService", function($scope, $state,$q, ngDialog,blockUI, WageFactory, WorkDairyApproveFactory, POScheduleItemsFactory, PlantRegisterService, WorkDairyDocketFactory, WorkDairyProgressFactory, EpsProjectSelectFactory,
				WorkDairyCopyFactory, UserService, $rootScope, WorkDairySubmitFactory, WorkDairyEmpFactory, WorkDairyPlantFactory, MaterialFactory, ProjectCrewPopupService,
				CreateWorkShiftPopupFactory, WorkDiaryService, ModuleUserFactory, WeatherService, CreateWeatherPopupFactory, WorkDairyCostCodeFactory, RegisterPurchaseOrderFactory,
				WorkDairyMaterialProjDocketFactory, GenericAlertService,WorkDairyAddProgressFactory, RequestForAdditionalTimeFactory, MaterialRegisterService) {
			...
			$scope.getProjSettingsForWorkDairy = function(projId) {
				...
				WorkDiaryService.getWorkDairyForInternalApproval(workDairyReq).then(
						function(data) {
							...
							if ($scope.workDairySearchReq.apprStatus != undefined && $scope.workDairySearchReq.apprStatus != null
									&& ($scope.workDairySearchReq.apprStatus == 'SubmittedForClientApproval' || $scope.workDairySearchReq.apprStatus == 'Approved')) {
			...
			$scope.approveWorkDiaryTabs = [ {
				...
				approveWorkdairySeleniumLocator:'AsbuiltRecords_WorkDiary_ApproveWorkDiary_progress'
			}, {
				title : 'Manpower Utilisation',
				url : 'views/timemanagement/workdairy/approveworkdairy/workdairyapprovemanpower.html',
				approveWorkdairySeleniumLocator:'AsbuiltRecords_WorkDiary_ApproveWorkDiary_ManpowerUtilization'
			
			}, {
				title : 'Plant Utilisation',
				url : 'views/timemanagement/workdairy/approveworkdairy/workdairyapproveplants.html',
				approveWorkdairySeleniumLocator:'AsbuiltRecords_WorkDiary_ApproveWorkDiary_PlantUtilization'
			
			}, {
				title : 'Material and Store Consumption',
				url : 'views/timemanagement/workdairy/approveworkdairy/workdairyapprovematerials.html',
				approveWorkdairySeleniumLocator:'AsbuiltRecords_WorkDiary_ApproveWorkDiary_MaterialandStoreConsumption'
			...
			$scope.getMaterialDeliveryDockets = function() {
				...
				angular.forEach($scope.workDairyMaterialDtlTOs, function(value, key) {
					$scope.materialSchReq.purchaseSchLabelKeyTO=value.purchaseSchLabelKeyTO;
					if (value.id <= 0 && value.docketType == 'Delivery Docket') {
						var purchaseSchLabelKeyTO = {};
						$scope.materialSchReq.purchaseSchLabelKeyTO = value.purchaseSchLabelKeyTO;
						$scope.materialSchReq.purchaseSchLabelKeyTO.displayNamesMap.purId = value.purchaseLabelKeyTO.id;
						$scope.materialSchReq.purchaseSchLabelKeyTO.displayNamesMap.purCode = value.purchaseLabelKeyTO.code;
						$scope.materialSchReq.projId=$scope.workDairySearchReq.projectLabelKeyTO.projId,
						$scope.materialSchReq.id=value.scheduleItemId
						$scope.materialSchReq = {
						...
						if ($scope.materialProjDtlMap[value.scheduleItemId] == null) {
							$scope.materialProjDtlMap[value.scheduleItemId] = value;
						}

					}
					saveMaterialreq.push({"id" : value.id,
						"status" : 1,
						"workDairyId" : $scope.workDairySearchReq.workDairyId,
						...
						"materialProjDtlTO" : $scope.materialSchReq,
			...
			$scope.resetCreateWorkDairy = function() {
				...
				$scope.workDairySearchReq = {
					...
					"apprStatus" : null,
					"workDairyDate" : null,
					"type" : 'SubmittedForClientApproval'  
				};
			...
			$scope.checkProgressQuantity=function(quantity,progress){
				...
				progress.errorFlag = false;
					...
					if (inputvalue > quantity) {
						...
						GenericAlertService
								.alertMessage(
										'WorkDairy cannot be booked more tham SOW Item Quantity',
										"Warning");
						return;
					}
					progress.errorFlag = false;
			...
			HelpService.pageHelp = function () {
				...
				helppagepopup = ngDialog.open({
					template: 'views/help&tutorials/requestapprovalhelp/approvalshelp/approveworkdairyhelp.html',
