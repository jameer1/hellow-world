...
app.config(["$stateProvider", function($stateProvider) {
	$stateProvider.state("notifyprojectscheduleofrates", {
		url : '/notifyprojectscheduleofrates',
		data : {
			roles : []
		},
		views : {
			'content@' : {
				templateUrl : 'views/notifications/projectscheduleofrates.html',
				controller : 'NotifyProjectScheduleofRates'
			}
		}
	})
}]).controller("NotifyProjectScheduleofRates", ["$rootScope", "$scope", "$q", "$state", "uiGridGroupingConstants", "uiGridConstants", "stylesService", "ngGridService", "$filter", "ngDialog", "GenericAlertService", "NotificationService", "EpsProjectMultiSelectFactory", function($rootScope, $scope, $q, $state, uiGridGroupingConstants, uiGridConstants, stylesService, ngGridService, $filter, ngDialog, GenericAlertService, NotificationService, EpsProjectMultiSelectFactory) {
	...
	$scope.getWorkDiaryNotificationDetails = function(){
		var req = {
			"projId" : $scope.searchProject.projIds,
			"status" : 1,
