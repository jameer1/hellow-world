...
app.config(["$stateProvider", function($stateProvider) {
	$stateProvider.state("notifyprojectresourcebudgetestimate", {
		url : '/notifyprojectresourcebudgetestimate',
		data : {
			roles : []
		},
		views : {
			'content@' : {
				templateUrl : 'views/notifications/projectresourcebudgetestimate.html',
				controller : 'NotifyProjectResourceBudgetEstimate'
			}
		}
	})
}]).controller("NotifyProjectResourceBudgetEstimate", ["$rootScope", "$scope", "$q", "$state", "uiGridGroupingConstants", "uiGridConstants", "stylesService", "ngGridService", "$filter", "ngDialog", "GenericAlertService", "NotificationService", "EpsProjectMultiSelectFactory", function($rootScope, $scope, $q, $state, uiGridGroupingConstants, uiGridConstants, stylesService, ngGridService, $filter, ngDialog, GenericAlertService, NotificationService, EpsProjectMultiSelectFactory) {
	...
	$scope.getWorkDiaryNotificationDetails = function(){
		...
		NotificationService.getCountNotification(req1).then(function(data){
			console.log(data);
		});
		var req = {
	...
	function (newValue, oldValue) {
		...
		if (newValue) {
			let columnDefs = [
				...
				{
					name: 'plant', headerCellClass: "justify-left", footerCellClass: "justify-left",
