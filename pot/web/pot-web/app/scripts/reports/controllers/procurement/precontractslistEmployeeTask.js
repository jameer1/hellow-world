...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("precontractslistEmployeeTask", {
		url: '/precontractslistEmployeeTask',
		data: {
			roles: []
		},
		views: {
			'content@': {
				templateUrl: 'views/reports/procurement/precontractslistEmployeeTask.html',
				controller: 'precontractslistEmployeeTaskController'
			}
		}
	})
}]).controller("precontractslistEmployeeTaskController", ["$scope", "$filter", "$q", "$state", "ngDialog", "EmpAttendanceService", "GenericAlertService", "PreContractInternalService","EpsProjectMultiSelectFactory", "MultipleCrewSelectionFactory", "generalservice", "stylesService", "ngGridService", "chartService", "uiGridConstants",
	...
	$scope.toDate = $filter('date')((defaultToDate), "dd-MMM-yyyy");    
		...
		function noSubReportData() {
			let columnDefs = [
				...
				{ field: 'purchaseOrderStatus', displayName: "Responsible Employee", headerTooltip: "Responsible Employee" },
		...
		$scope.getEmpTaskReports = function(){
		var req = {
			"status" : 1,
		...
		$scope.searchReq = req;
		//editPreContractList = [];
		PreContractInternalService.getPreContractListEmpTaskReport(req).then(function(data) {
		console.log(data);
		...
		$scope.$watch(function () { return stylesService.finishedStyling; },
			function (newValue, oldValue) {
				...
				{ field: 'estimateTotalCost', displayName: "Budget Estimate", headerTooltip: "Budget Estimate" },
				{ field: 'resEmp', displayName: "Responsible Employee", headerTooltip: "Responsible Employee" },						
					];
					let data = [];
					$scope.gridOptions = ngGridService.initGrid($scope, columnDefs, data);
				}
			});
		$scope.resetEmpTaskData = function () {
			$scope.plantDetails = [];
			$scope.data = [];
			$scope.labels = [];
			$scope.selectedProjIds = [];
