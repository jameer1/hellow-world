...
app.config(["$stateProvider", function ($stateProvider) {
    $stateProvider.state("revenuethroughassetsales", {

        url: '/revenuethroughassetsales',
        data: {
            roles: []
        },
        views: {
            'content@': {
                templateUrl: 'views/finance/financemain/immovableassets/revenuethroughassetsales.html',
                controller: 'RevenueThroughAssetSalesController'
            }
        }
    })
}]).controller("RevenueThroughAssetSalesController", ["$scope", "$q", "$state", "ngDialog", "UserEPSProjectService", "GenerateInvoiceFactory", "GenericAlertService", "ProfitCentrePopUpFactory", "AssetCategorySelectFactory", "RevenueAssetSalesFactory", function ($scope, $q, $state, ngDialog, UserEPSProjectService, GenerateInvoiceFactory, GenericAlertService, ProfitCentrePopUpFactory, AssetCategorySelectFactory, RevenueAssetSalesFactory) {
    ...
    $scope.addAssetSales = function(actionType) {
		if (actionType == 'Edit') {
			GenericAlertService.alertMessage("Please select atleast one row to modify", 'Warning');
			return;
		}
		var addAssetSalesPopup = RevenueAssetSalesFactory.addAssetSalesPopup(actionType);
		addAssetSalesPopup.then(function(data) {
