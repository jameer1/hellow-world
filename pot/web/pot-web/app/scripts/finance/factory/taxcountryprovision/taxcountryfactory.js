			...
			controller : [ '$scope', function($scope) {
				...
				$scope.taxDetails = function(taxcode) {
					...
					taxPopup.then(function(data) {
						...
						GenericAlertService.alertMessage("Error occured while selecting project taxcountry Details", 'Error');
					});
				}
				...
				$scope.savetaxcode = function() {
					...
					TaxCodeService.saveTaxCalculationCode(req).then(function(data) {
						blockUI.stop();
						GenericAlertService.alertMessage('TaxCountry New Request Detail(s) is/are  ' + data.message, data.status);
					}, function(error) {
						blockUI.stop();
						GenericAlertService.alertMessage('TaxCountry New Request Detail(s) is/are Failed to Save ', "Error");
					});
					ngDialog.close();
				}
			} ]
