...
app
		.factory(
				'PreContractBidanalysisRequestFactory',
				["ngDialog", "$q", "$filter", "$timeout", "$rootScope", "PreContractExternalService", "ApproveUserFactory", "GenericAlertService", "generalservice", function(ngDialog, $q, $filter, $timeout, $rootScope,
						PreContractExternalService, ApproveUserFactory,GenericAlertService,generalservice) {
					...
					service.getCompanyBidDetails = function(
						...
						viewBidderFactoryPopUp = ngDialog
								.open({
									template : 'views/procurement/pre-contracts/externalApproval/precontractbidanalysisrequest.html',
											...
											var currentDate = $scope.preContractData.currentDate;
												...
												$scope.expandCollapseSow=function(sowFlag)
												{
													$scope.sowFlag=!sowFlag;
																	...
																	.then(
																			function(
																				...
																				if (apprvStatus == 2) {
																					...
																					succMsg = GenericAlertService
																							.alertMessageModal(
																									"Bid send back to requestor successfully ",
																									"Info");

																				} else if (apprvStatus == 4) {
																					succMsg = GenericAlertService
																							.alertMessageModal(
																									"Bid put on-hold successfully ",
																									"Info");
																				} else if (apprvStatus == 5) {
																					succMsg = GenericAlertService
																							.alertMessageModal(
																									"Bid got approved successfully ",
																									"Info");
																				} else if (apprvStatus == 6) {
																					succMsg = GenericAlertService
																							.alertMessageModal(
																									"Bid got rejected successfully ",
																									"Info");
																				}
														...
														$scope.getApproverDetails = function(approverTo) {
															var getReq = {
																"moduleCode" : "EXTERNALREQAPPOVAL",
																"actionCode" : "APPROVE",
																"permission" : "PROCURMT_EXTRNLSTAGE2APRVL_APPROVE",
														...
														$scope.sendForApproval = function(
															...
															var closeDate =null;
															if ($scope.preContractData.revisedCloseDate != undefined && $scope.preContractData.revisedCloseDate != null) {
																closeDate = new Date($scope.preContractData.revisedCloseDate);
															} else {
																closeDate = new Date($scope.preContractData.closeDate);
															}
															if (currentDate > closeDate) {
																GenericAlertService.alertMessage('Precontract cannot be submmitted before tender close date', 'Warning');
																return;
															}
															if($scope.approverTo.name ==null && $scope.approverTo.name == undefined){
														...
														$scope.validateBid=function()
														{
															...
															if ($scope.preContractData.precontractSowDtlTOs) {
																angular
																.forEach(
																		$scope.preContractData.precontractSowDtlTOs,
																		function(
																				sowDtlTO,
																				key) {
																			var quantity = 0;
																									...
																									if(count > 0)
																										{
																											if(sowCmpTO.quantity <=0)
																												...
																												GenericAlertService
																												.alertMessage(
																														"Please enter  scope of work items approval quantity for bid",
																														"Error");
																												forEach.break();
																												apprvQuantity=0;
																												}
																			...
																			if (sowDtlTO.quantity < quantity) {
																				GenericAlertService
																						.alertMessage(
																								"Scope of work Approval quantity cannot be greater than actual quantity",
																								"Warning");
																				forEach.break();
																			}
																			...
																			if (!sowFalg) {
																				GenericAlertService
																				.alertMessage(
																						"Please select scope of work item for bid for approval",
																						"Warning");
																				forEach.break();
