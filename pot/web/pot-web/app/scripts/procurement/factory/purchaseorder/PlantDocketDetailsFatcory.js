...
app.factory('PlantDocketDetailsFactory', ["ngDialog", "$q", "$filter", "$timeout", "$rootScope", "blockUI", "PurchaseOrderService", "PreContractInternalService", "GenericAlertService", function(ngDialog, $q, $filter, $timeout, $rootScope, blockUI,PurchaseOrderService,PreContractInternalService, GenericAlertService) {
	var service = {};
	service.getPlantDeliveryDetails = function(item,companyMap,selectedPurchaseOrderData) {
	...
	service.openPopup = function(item,companyMap,selectedPurchaseOrderData,labelKeyTOs) {
		...
		var resourcePopup = ngDialog.open({
			template : 'views/procurement/purchaseorders/purchaseplantdeliverydocket.html',
