...
app.config(["$stateProvider", function($stateProvider) {
	$stateProvider.state("poexternalapproval", {
		url : '/poexternalapproval',
		data : {
			roles : []
		},
		views : {
			'content@' : {
				templateUrl : 'views/procurement/purchaseorders/poexternalapproval.html',
			}
		}
	})
}]).controller("PurchaseExternalApprovalController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "PreContractExternalFactory", "PurchaseBidanalysisFactory", "PrecontractReqApprovalFactory", "PreContractExternalService", "GenericAlertService","stylesService", "ngGridService", function($rootScope, $scope, $q, $state, ngDialog,PreContractExternalFactory, PurchaseBidanalysisFactory,PrecontractReqApprovalFactory,PreContractExternalService, GenericAlertService, stylesService, ngGridService) {
	...
	$scope.$watch(function () { return stylesService.finishedStyling; },
			function (newValue, oldValue) {
				if (newValue) {
					...
					let data = [];
					$scope.gridOptions = ngGridService.initGrid($scope, columnDefs, data, "Procurement_purchaseOrderList_Stage2Approval");
				}
		});
}]);
