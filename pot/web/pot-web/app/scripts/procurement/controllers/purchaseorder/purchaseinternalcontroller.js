...
app.config(["$stateProvider", function($stateProvider) {
	$stateProvider.state("pointernal", {
		url : '/pointernal',
		data : {
			roles : []
		},
...
$scope.$on("resetStage1request&appoval", function() {
...
$scope.$watch(function () { return stylesService.finishedStyling; },
			function (newValue, oldValue) {
				if (newValue) {
					...
					let data = [];
					$scope.gridOptions = ngGridService.initGrid($scope, columnDefs, data, "Procurement_purchaseOrderList_Stage1Request");
					$scope.getInternalPrecontractDetails();
				}
		});
}]);