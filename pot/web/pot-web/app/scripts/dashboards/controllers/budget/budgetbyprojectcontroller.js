...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("budgetbyproject", {
		url: '/budgetbyproject',
		data: {
			dashboards: []
		},
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/budget/budgetbyprojectfilter.html',
				controller: 'BudgetByProjectController'
			}
		}
	})
}]).controller("BudgetByProjectController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	"ProjectStatusService", "ProjectSettingsService", "EstimateToCompleteDashboardGroupingService",  function($rootScope, $scope, $q, $state, ngDialog, GenericAlertService, UserEPSProjectService, 
		...
		$scope.getBudgetByProjectDetails = function () {
			if ($scope.budgetType.code === "projectManpower") {
				...
				function mergeVal(estAtCompByProject) {
					...
					for (let i=0; i < estAtCompByProject.length; i++) {
						...
						if (index == -1) {
							newData.push({"code": estAtCompByProject[i].code, "budget" : estAtCompByProject[i].originalHrsTotal});
						} else {
							newData[index].budget += estAtCompByProject[i].originalHrsTotal;
						}
					}
					console.log(newData)
			...
			function mergeVal(estAtCompByCountry) {
				...
				for (let i=0; i < estAtCompByCountry.length; i++) {
					...
					if (index == -1) {
						newData.push({"code": estAtCompByCountry[i].code, "budget" : estAtCompByCountry[i].originalBudgetTotal});
					} else {
						newData[index].budget += estAtCompByCountry[i].originalBudgetTotal;
					}
				}
				console.log(newData)
				setGraphData(newData);
		...
		/*
		$scope.getBudgetByProjectDetails = function () {
			BudgetDashboardGroupingService.getBudgetDashboards($scope.selectedProjIds, $scope.budgetType.code).then(data => {
		...
		*/
		...
		$scope.options.scales.xAxes[0].ticks.reverse = true;
		$scope.options.scales.yAxes[0].ticks.reverse = true;
		}]];
		var HelpService = {};
		...
		HelpService.pageHelp = function () {
			...
			helppagepopup = ngDialog.open({
				template: 'views/help&tutorials/dashboardshelp/budget/budgetbyprojecthelp.html',
