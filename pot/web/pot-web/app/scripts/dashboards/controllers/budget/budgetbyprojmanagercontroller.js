...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("budgetbyprojectmanager", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/budget/budgetbyprojectmanagerfilter.html',
				controller: 'BudgetByProjectManagerController'
			}
		}
	})
}]).controller("BudgetByProjectManagerController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getBudgetByProjectManager = function () {
	...
	function mergePlannedValue(progVarienceDataInfo, plannedValuesMap) { 
		...
		if (taskPlannedValue && taskFull) {
			...
			$scope.budgetByProjectManagerDetails = new Array();
			for (const index_3 in budgetDetails){
				...
				$scope.budgetByProjectManagerDetails.push(budgetDetails[index_3]);
				
			}
			mergeValForProjectManager($scope.budgetByProjectManagerDetails);
		}
	}
	...
	/*
	$scope.getBudgetByProjectManager = function () {
		BudgetDashboardGroupingService.getBudgetDashboards($scope.selectedProjIds, "projectManager").then(data => {
			setGraphData(data);
		});
	};
	*/
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/budget/budgetbyprojectmanagerhelp.html',
