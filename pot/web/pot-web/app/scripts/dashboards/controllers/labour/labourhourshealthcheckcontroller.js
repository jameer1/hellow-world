...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("labourhourhealthcheck", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/labour/labourhourshealthcheckfilter.html',
				controller: 'LabourHrsHealthCheckController'
			}
		}
	})
}]).controller("LabourHrsHealthCheckController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	"ProjectStatusService", "ProjectSettingsService", "stylesService", "ngGridService","ResourceCurveService",
	function($rootScope, $scope, $q, $state, ngDialog, GenericAlertService, UserEPSProjectService, 
			$location,PerformanceDashboardService, EpsProjectMultiSelectFactory, TreeService, EstimateToCompleteService, ProjectScheduleService, 
			ProjectStatusService, ProjectSettingsService,stylesService, ngGridService, ResourceCurveService) {
	...
	$scope.getLabourHrsHealthCheckDetails = function () {
		...
		for (const index_3 in costData) {
			for (const index_4 in manPowerData) {
				if(manPowerData[index_4].projId == costData[index_3].projId) {
					...
					costData[index_3].progPercentageVarienceDirManHrs = ((costData[index_3].plannedDirectManHours / manPowerData[index_4].originalHrsTotal)*100) - ((manPowerData[index_4].earnedManHoursTotal / manPowerData[index_4].originalHrsTotal)*100);
					...
					// To Complete Performance Index for LABOUR
					if(costData[index_3].tcpiManHrs >= costData[index_3].exTcpiUpperLimit
							&& costData[index_3].tcpiManHrs < costData[index_3].exTcpiLowerLimit) {
						costData[index_3].TcpiManPowerExceptional = true;
					}
					if(costData[index_3].tcpiManHrs >= costData[index_3].acTcpiUpperLimit
							&& costData[index_3].tcpiManHrs < costData[index_3].acTcpiLowerLimit) {
						costData[index_3].TcpiManPowerAcceptable = true;
					}
					if(costData[index_3].tcpiManHrs >= costData[index_3].waTcpiUpperLimit
							&& costData[index_3].tcpiManHrs <= costData[index_3].waTcpiLowerLimit) {
						costData[index_3].TcpiManPowerWarning = true;
					}
					if(costData[index_3].tcpiManHrs >= costData[index_3].crTcpiLowerLimit) {
...
//CPI
 ...
 //TCPI
		...
		console.log("labourHealthCheck", $scope.labourHealthCheck);
		$scope.gridOptions.data = angular.copy($scope.labourHealthCheck);
	}
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/labour/labourhrshealthcheckhelp.html',
	...
	$scope.$watch(function () { return stylesService.finishedStyling;}, 
           ...
           function (newValue, oldValue) {
			if (newValue) {
				...
				let data = [];
				$scope.gridOptions = ngGridService.initGrid($scope, columnDefs, data,"Dashboard_Labour_Labour Health Check");	
			}
		});
}]);
