...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("estimateatcompletionbyprojectmanager", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/estimateatcompletion/estimateatcompletionbyprojmanagerfilter.html',
				controller: 'EstimateAtCompletionByProjManagerController'
			}
		}
	})
}]).controller("EstimateAtCompletionByProjManagerController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getEstimateAtProjManagerDetails = function () {
			...
			for (const index_3 in estAtCompletionDetails){
				for (const index_4 in projGeneral) {
					if(projGeneral[index_4].projId == estAtCompletionDetails[index_3].projId) {
						estAtCompletionDetails[index_3].name = projGeneral[index_4].userLabelKeyTO.name
					}
				}
	...
	function mergeVal(estAtCompByCountry) {
		...
		for (let i=0; i < estAtCompByCountry.length; i++) {
			let index = newData.findIndex(e => e.name == estAtCompByCountry[i].name);
			console.log(estAtCompByCountry[i].name);
			if (index == -1) {
				newData.push({"name": estAtCompByCountry[i].name, "estimation" : estAtCompByCountry[i].estAtCompletionTotal});
			} else {
	...
	*/
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/estatcompletion/estatcompletionbyprojmanagerhelp.html',
