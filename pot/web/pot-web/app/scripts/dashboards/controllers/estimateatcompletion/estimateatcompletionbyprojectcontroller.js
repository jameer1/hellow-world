...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("estimateatcompletionbyproject", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/estimateatcompletion/estimateatcompletionbyprojectfilter.html',
				controller: 'EstimateAtCompletionByProjectController'
			}
		}
	})
}]).controller("EstimateAtCompletionByProjectController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	"ProjectStatusService", "ProjectSettingsService", "EstimateToCompleteDashboardGroupingService",  function($rootScope, $scope, $q, $state, ngDialog, GenericAlertService, UserEPSProjectService, 
		...
		$scope.estimateTypes = [{ name: 'Manpower', code: 'projectManpower' }, { name: 'Cost', code: 'projectCost' }];
		$scope.estimateType = $scope.estimateTypes[1];
		...
		$scope.getEstimateAtProjectDetails = function () {
			if ($scope.estimateType.code === "projectManpower") {
				...
				function mergeVal(estAtCompByProject) {
					...
					for (let i=0; i < estAtCompByProject.length; i++) {
						let index = newData.findIndex(e => e.code == estAtCompByProject[i].code);
						if (index == -1) {
							newData.push({"code": estAtCompByProject[i].code, "estimation" : estAtCompByProject[i].estAtCompletionHrsTotal});
						} else {
							newData[index].estimation += estAtCompByProject[i].estAtCompletionHrsTotal;
						}
					}
					console.log(newData)
			...
			function mergeVal(estAtCompByCountry) {
				...
				for (let i=0; i < estAtCompByCountry.length; i++) {
					let index = newData.findIndex(e => e.code == estAtCompByCountry[i].code);
					if (index == -1) {
						newData.push({"code": estAtCompByCountry[i].code, "estimation" : estAtCompByCountry[i].estAtCompletionTotal});
					} else {
						newData[index].estimation += estAtCompByCountry[i].estAtCompletionTotal;
					}
				}
				console.log(newData)
				setGraphData(newData);
		...
		$scope.estimateCompletionByProjTabs = new Array();
		$scope.getEstimateAtProjectDetails = function () {
			EstimateToCompleteDashboardGroupingService.getEstimateToCompleteDashboards($scope.selectedProjIds, $scope.estimateType.code)
				.then(data => {
					setGraphData(data);
				});
		};
		*/
		function setGraphData(estimateDetails) {
			...
			for (const key in estimateDetails) {
				...
				data.push(parseFloat(estimateDetails[key].estimation).toFixed(2));
			}
		...
		HelpService.pageHelp = function () {
			...
			helppagepopup = ngDialog.open({
				template: 'views/help&tutorials/dashboardshelp/estatcompletion/estatcompletionbyprojecthelp.html',
