...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("estimateatcompletionbycountry", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/estimateatcompletion/estimateatcompletionbycountryfilter.html',
				controller: 'EstimateAtCompletionByCountryController'
			}
		}
	})
}]).controller("EstimateAtCompletionByCountryController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	/*"EstimateAtCompletionByCountryController", ["$scope","$q", "ngDialog", "GenericAlertService",
		"EstimateAtCompletionService", "EpsProjectMultiSelectFactory", "EstimateToCompleteDashboardGroupingService", function ($scope,$q, ngDialog, GenericAlertService,
			EstimateAtCompletionService, EpsProjectMultiSelectFactory, EstimateToCompleteDashboardGroupingService*/
	...
	$scope.getEstimateAtCountryDetails = function () {
	...
	function mergeVal(estAtCompByCountry) {
		...
		for (let i=0; i < estAtCompByCountry.length; i++) {
			let index = newData.findIndex(e => e.countryName == estAtCompByCountry[i].countryName);
			if (index == -1) {
				newData.push({"countryName": estAtCompByCountry[i].countryName, "estimation" : estAtCompByCountry[i].estAtCompletionTotal});
			} else {
	...
	*/
	...
	function setGraphData(estimateDetails) {
		...
		for (const key in estimateDetails) {
			//labels.push(estimateDetails[key].code);
			labels.push(estimateDetails[key].countryName);
			//data.push(parseFloat(estimateDetails[key].estimateCompletion).toFixed(2));
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/estatcompletion/estatcompletionbycountryhelp.html',
