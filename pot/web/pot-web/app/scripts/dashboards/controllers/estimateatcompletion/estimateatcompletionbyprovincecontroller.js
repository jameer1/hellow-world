...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("estimateatcompletionbyprovince", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/estimateatcompletion/estimateatcompletionbyprovisionfilter.html',
				controller: 'EstimateAtCompletionByProvisionController'
			}
		}
	})
}]).controller("EstimateAtCompletionByProvisionController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getEstimateAtProvinceDetails = function () {
	...
	function mergeVal(estAtCompByCountry) {
		...
		for (let i=0; i < estAtCompByCountry.length; i++) {
			...
			if (index == -1) {
				newData.push({"province": estAtCompByCountry[i].provisionName, "estimation" : estAtCompByCountry[i].estAtCompletionTotal});
			} else {
	...
	*/
	function setGraphData(estimateDetails) {
		...
		for (const key in estimateDetails) {
			...
			labels.push(estimateDetails[key].province);
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/estatcompletion/estatcompletionbyprovincehelp.html',
