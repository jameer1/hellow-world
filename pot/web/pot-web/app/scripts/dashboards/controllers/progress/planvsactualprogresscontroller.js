...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("planvsactualprogress", {
		...
		views: {
			'content@': {
				...
				'templateUrl': 'views/dashboards/progress/planvsactualprogressfilter.html',
				controller: 'PlanVsActualProgressController'
			}
		}
	})
}]).controller("PlanVsActualProgressController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	"ProjectStatusService", 'stylesService', 'ngGridService', "ProjectSettingsService", function($rootScope, $scope, $q, $state, ngDialog, GenericAlertService, UserEPSProjectService, 
			$location,PerformanceDashboardService, EpsProjectMultiSelectFactory, TreeService, EstimateToCompleteService, ProjectScheduleService, 
			ProjectStatusService, stylesService, ngGridService, ProjectSettingsService) {
	...
	function mergePlannedValue(progVarienceDataInfo, plannedValuesMap) { 
		if (taskPlannedValue && taskFull) {
			...
			for (const index_1 in progVarienceDataInfo) {
				...
				$scope.actualPlannedProgressData.push(progVarienceDataInfo[index_1]);
			}
			$scope.gridOptions.data = angular.copy($scope.actualPlannedProgressData);
			console.log("FINAL", $scope.actualPlannedProgressData);
		}
	}
			...
			function (newValue, oldValue) {
				if (newValue) {
					let columnDefs = [
						...
						{
							name: 'actualProgress',
							cellTemplate: '<div>{{row.entity.actualProgress | number: 2 }}</div>',
							displayName: "Actual Progress", headerTooltip: 'Actual Progress'
						},
						{
							name: 'plannedProgress',
							cellTemplate: '<div>{{row.entity.plannedProgress | number: 2 }}</div>',
							displayName: "Planned Progress", headerTooltip: 'Planned Progress'
						},
						
						
					]
					let data = [];
					$scope.gridOptions = ngGridService.initGrid($scope, columnDefs, data, "Dashboard_Progress_PlanVsActualProgress");
				}
			});
