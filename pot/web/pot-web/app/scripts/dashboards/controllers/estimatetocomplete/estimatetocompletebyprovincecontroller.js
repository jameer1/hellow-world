...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("estimatetocompletebyprovince", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/estimatetocomplete/estimatetocompletebyprovincefilter.html',
				controller: 'EstimateToByProvinceController'
			}
		}
	})
}]).controller("EstimateToByProvinceController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getEstimateToProvinceDetails = function () {
	...
	function mergeVal(budgetByCountry) {
		...
		for (let i=0; i < budgetByCountry.length; i++) {
			let index = newData.findIndex(e => e.provisionName == budgetByCountry[i].provisionName);
			if (index == -1) {
				newData.push({"provisionName": budgetByCountry[i].provisionName, "estToComplete" : budgetByCountry[i].estimateCompletion});
			} else {
	...
	$scope.getEstimateToProvinceDetails = function () {
		...
		EstimateToCompleteDashboardGroupingService.getEstimateToCompleteDashboards($scope.selectedProjIds, "province")
			.then(data => {
				setGraphData(data);
			});
	};
	*/
	...
	function setGraphData(estimateDetails) {
		...
		for (const key in estimateDetails) {
			labels.push(estimateDetails[key].provisionName);
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/esttocomplete/esttocompletebyprovincehelp.html',
