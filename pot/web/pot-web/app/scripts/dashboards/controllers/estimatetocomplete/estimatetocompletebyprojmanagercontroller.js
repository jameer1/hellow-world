...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("estimatetocompleteprojectmanager", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/estimatetocomplete/estimatetocompletebyprojmanagerfilter.html',
				controller: 'EstimateToByProjManagerController'
			}
		}
	})
}]).controller("EstimateToByProjManagerController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getEstimateToProjManagerDetails = function () {
	...
	function mergeVal(budgetByCountry) {
		...
		for (let i=0; i < budgetByCountry.length; i++) {
			let index = newData.findIndex(e => e.name == budgetByCountry[i].name);
			if (index == -1) {
				newData.push({"name": budgetByCountry[i].name, "estToComplete" : budgetByCountry[i].estimateCompletion});
			} else {
				newData[index].estToComplete += budgetByCountry[i].estimateCompletion;
			}
		}
		console.log(newData)
	...
	/*
	$scope.getEstimateToProjManagerDetails = function () {
		EstimateToCompleteDashboardGroupingService.getEstimateToCompleteDashboards($scope.selectedProjIds, "projectManager")
			.then(data => {
				setGraphData(data);
			});
	};
	*/
	...
	function setGraphData(estimateDetails) {
		...
		for (const key in estimateDetails) {
			labels.push(estimateDetails[key].name);
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/esttocomplete/esttocompletebyprojmanagerhelp.html',
