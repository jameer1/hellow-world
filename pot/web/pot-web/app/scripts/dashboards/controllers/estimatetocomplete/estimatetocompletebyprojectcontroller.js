...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("estimatetocompletebyproject", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/estimatetocomplete/estimatetocompletebyprojectfilter.html',
				controller: 'EstimateToByProjectController'
			}
		}
	})
}]).controller("EstimateToByProjectController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getEstimateToProjectDetails = function () {
		if($scope.budgetType.code == "projectManpower"){
task1Completed = false;
			...
			PerformanceDashboardService.getPerformanceIndex(req).then(function (data) {
				console.log("data.projManpowerTOs",data);
			...
			function mergingValues(projManPowerValues, plannedValuesMap) {
				if (task1Completed && taskPlannedValue) {
					...
					for (const index_3 in estAtCompletionDetails){
						for (const index_4 in projGeneral) {
							if(projGeneral[index_4].projId == estAtCompletionDetails[index_3].projId) {
								estAtCompletionDetails[index_3].code = projGeneral[index_4].projId
							}
						}
					...
					for(let i=0;i<$scope.estimateAtCompByProject.length;i++){
						if($scope.estimateAtCompByProject[i].estimateType == "(BAC-EV)/(PF*SPI)"){
							$scope.estimateAtCompByProject[i].etcTotal = (($scope.estimateAtCompByProject[i].revisedHrsTotal>0?$scope.estimateAtCompByProject[i].revisedHrsTotal:$scope.estimateAtCompByProject[i].originalHrsTotal)-$scope.estimateAtCompByProject[i].earnedManHoursTotal)/(($scope.estimateAtCompByProject[i].earnedManHoursTotal/$scope.estimateAtCompByProject[i].actualHrsTotal)*($scope.estimateAtCompByProject[i].earnedManHoursTotal/$scope.estimateAtCompByProject[i].plannedManHoursTotal));
						}
			...
			function mergeVal(estAtCompByProject) {
				console.log("estAtCompByProject", estAtCompByProject);
				let newData = [];
				for (let i=0; i < estAtCompByProject.length; i++) {
					let index = newData.findIndex(e => e.projId == estAtCompByProject[i].projId);
					if (index == -1) {
						newData.push({"code": estAtCompByProject[i].code, "estToComplete" : estAtCompByProject[i].etcTotal});
					} else {
						newData[index].budget += estAtCompByProject[i].etcTotal;
					}
				}
				console.log(newData)
			...
			function calculateManhoursTotal(projManpowerDetails) {
				...
				for (const value of projManpowerDetails) {
					...
					for (let i=0; i < projManPowerValues.length; i++) {
						...
						if (projManPowerValues[i].projId == value.projId) {
							...
							if(value.estimateType == "Remaining Units") {
								console.log(value.earnedValue);
								console.log(value.originalQty);
								console.log(value.actualQty);
								if(value.revisedQty > 0) {
					...
					if (!found) {
						...
						//projManPowerValues.estimateType = value.estimateType;
		...
		ProjectSettingsService.getProjPlannedValue(req).then(function (data) {
			$rootScope.plannedValuesMap = data.projPlannedValueTO;
			for(let i=0;i<$scope.plannedValuesMap.length;i++){
			$scope.plan.push($scope.plannedValuesMap[i].plannedValues);
			}
	...
	function mergePlannedValue(progVarienceDataInfo, plannedValuesMap) { 
		...
		if (taskPlannedValue && taskFull) {
			...
			var budgetDetails = $scope.costSchedPerformanceData;
			var projGeneral = $scope.projGeneralValues;
			$scope.budgetByCountryDetails = new Array();
			for (const index_3 in budgetDetails){
				...
				$scope.budgetByCountryDetails.push(budgetDetails[index_3]);
					/*for(let i=0;i<$scope.budgetByCountryDetails.length;i++){
					if($scope.budgetByCountryDetails[i].estimateType == "Remaining Units"){
					if($scope.budgetByCountryDetails[i].revisedBudgetTotal > 0){
					($scope.budgetByCountryDetails[i]).estimateCompletion = ($scope.budgetByCountryDetails[i].revisedBudgetTotal)-($scope.budgetByCountryDetails[i].actualCostTotal);
				}else{
					($scope.budgetByCountryDetails[i]).estimateCompletion = ($scope.budgetByCountryDetails[i].originalBudgetTotal)-($scope.budgetByCountryDetails[i].actualCostTotal);
				}
				}else if($scope.budgetByCountryDetails[i].estimateType == "BAC-EV"){
					if($scope.budgetByCountryDetails[i].revisedBudgetTotal > 0){
					($scope.budgetByCountryDetails[i]).estimateCompletion = ($scope.budgetByCountryDetails[i].revisedBudgetTotal)-($scope.budgetByCountryDetails[i].earnedValueTotal);
				}else{
					($scope.budgetByCountryDetails[i]).estimateCompletion = ($scope.budgetByCountryDetails[i].originalBudgetTotal)-($scope.budgetByCountryDetails[i].earnedValueTotal);
				}
				}else if($scope.budgetByCountryDetails[i].estimateType == "(BAC-EV)/CPI"){
					if($scope.budgetByCountryDetails[i].revisedBudgetTotal > 0){
					($scope.budgetByCountryDetails[i]).estimateCompletion = ($scope.budgetByCountryDetails[i].revisedBudgetTotal-$scope.budgetByCountryDetails[i].earnedValueTotal)/(($scope.budgetByCountryDetails[i].earnedValueTotal/$scope.budgetByCountryDetails[i].actualCostTotal));
				}else{
					($scope.budgetByCountryDetails[i]).estimateCompletion = ($scope.budgetByCountryDetails[i].originalBudgetTotal-$scope.budgetByCountryDetails[i].earnedValueTotal)/(($scope.budgetByCountryDetails[i].earnedValueTotal/$scope.budgetByCountryDetails[i].actualCostTotal));
				}
				}else if($scope.budgetByCountryDetails[i].estimateType == "(BAC-EV)/(CPI*SPI)"){
					if($scope.budgetByCountryDetails[i].revisedBudgetTotal > 0){
					($scope.budgetByCountryDetails[i]).estimateCompletion = ($scope.budgetByCountryDetails[i].revisedBudgetTotal-$scope.budgetByCountryDetails[i].earnedValueTotal)/(($scope.budgetByCountryDetails[i].earnedValueTotal/$scope.budgetByCountryDetails[i].actualCostTotal)*($scope.budgetByCountryDetails[i].earnedValueTotal/$scope.budgetByCountryDetails[i].plannedCost));
				}else{
					($scope.budgetByCountryDetails[i]).estimateCompletion = ($scope.budgetByCountryDetails[i].originalBudgetTotal-$scope.budgetByCountryDetails[i].earnedValueTotal)/(($scope.budgetByCountryDetails[i].earnedValueTotal/$scope.budgetByCountryDetails[i].actualCostTotal)*($scope.budgetByCountryDetails[i].earnedValueTotal/$scope.budgetByCountryDetails[i].plannedCost));
				}
				}else if($scope.budgetByCountryDetails[i].estimateType == "New Estimate"){
					($scope.budgetByCountryDetails[i]).estimateCompletion = $scope.budgetByCountryDetails[i].estCompletion;
				}
				}*/
			}
			mergeVal($scope.budgetByCountryDetails);
		}
	}
	
	function mergeVal(budgetByCountry) {
		console.log("budgetByCountry", budgetByCountry);
		let newData = [];
		for (let i=0; i < budgetByCountry.length; i++) {
			let index = newData.findIndex(e => e.code == budgetByCountry[i].code);
			if (index == -1) {
				newData.push({"code": budgetByCountry[i].code, "estToComplete" : budgetByCountry[i].etcTotal});
			} else {
				newData[index].estToComplete += budgetByCountry[i].etcTotal;
			}
		}
		console.log(newData)
		setGraphData(newData);
	...
	console.log($rootScope.plannedValuesMap);
		let projCostValues = [];
		let j=0;
			...
			for (let i=0; i < projCostValues.length; i++) {
				...
				if (projCostValues[i].projId == costValue.projId) {
					...
					if(costValue.estimateType == "Remaining Units") {
						...
						var cpi=(costValue.earnedValue==0 || costValue.earnedValue==null || actualCostTotal1==0) ? 0 : (costValue.earnedValue / actualCostTotal1);
						console.log("kjfncj");
						console.log($scope.plannedValuesMap);
						var spi=(costValue.earnedValue==0 || costValue.earnedValue==null || costValue.plannedValue==0) ? 0 : (costValue.earnedValue / $scope.plannedValuesMap[0].plannedValues[j]);
						j++;
						...
						console.log(projCostValues[i].estCompletion);
	...
	$scope.getEstimateToProjectDetails = function () {
		...
		EstimateToCompleteDashboardGroupingService.getEstimateToCompleteDashboards($scope.selectedProjIds, $scope.budgetType.code)
			.then(data => {
				setGraphData(data);
			});
	};
	*/
	...
	function setGraphData(estimateDetails) {
		const labels = new Array();
		const data = new Array();
		let index;
		for (const key in estimateDetails) {
			index = $scope.selectedProjIds.indexOf(estimateDetails[key].code);
			labels.push($scope.searchProject.projName.split(',')[index]);
			data.push(parseFloat(estimateDetails[key].estToComplete).toFixed(2));
		}
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/esttocomplete/esttocompletebyprojecthelp.html',
