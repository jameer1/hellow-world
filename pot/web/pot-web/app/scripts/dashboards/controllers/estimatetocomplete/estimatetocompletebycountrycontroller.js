...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("estimatetocompletebycountry", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/estimatetocomplete/estimatetocompletebycountryfilter.html',
				controller: 'EstimateToByCountryController'
			}
		}
	})
}]).controller("EstimateToByCountryController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getEstimateToCountryDetails = function () {
	...
	function mergeVal(budgetByCountry) {
		...
		for (let i=0; i < budgetByCountry.length; i++) {
			...
			if (index == -1) {
				newData.push({"countryName": budgetByCountry[i].countryName, "estToComplete" : budgetByCountry[i].estimateCompletion});
			} else {
	...
	/*
	$scope.getEstimateToCountryDetails = function () {
		EstimateToCompleteDashboardGroupingService.getEstimateToCompleteDashboards($scope.selectedProjIds, "country")
			.then(data => {
				setGraphData(data);
			});
	};
	*/
	function setGraphData(estimateDetails) {
		...
		for (const key in estimateDetails) {
			labels.push(estimateDetails[key].countryName);
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/esttocomplete/esttocompletebycountryhelp.html',
