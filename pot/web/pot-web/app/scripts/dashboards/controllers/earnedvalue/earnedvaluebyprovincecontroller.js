...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("earnedvaluebyprovince", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/earnedvalue/earnedvaluebyprovincefilter.html',
				controller: 'EarnedValuByProvinceController'
			}
		}
	})
}]).controller("EarnedValuByProvinceController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	"ProjectStatusService", "ProjectSettingsService", "EstimateToCompleteDashboardGroupingService",  function($rootScope, $scope, $q, $state, ngDialog, GenericAlertService, UserEPSProjectService, 
		...
		$scope.earnedValueTypes = [{ name: 'Manpower', code: 'provinceManpower' }, { name: 'Cost', code: 'provinceCost' }];
		...
		$scope.getEarnedValueByProvinceDetails = function () {
			if ($scope.earnedValueType.code === "provinceManpower") {
				...
				function mergeVal(earnedHrsByCountry) {
					...
					for (let i=0; i < earnedHrsByCountry.length; i++) {
						let index = newData.findIndex(e => e.provisionName == earnedHrsByCountry[i].provisionName);
						if (index == -1) {
							newData.push({"provisionName": earnedHrsByCountry[i].provisionName, "earnedValue" : earnedHrsByCountry[i].earnedManHoursTotal});
						} else {
			...
			function mergeVal(earnedValByCountry) {
				...
				for (let i=0; i < earnedValByCountry.length; i++) {
					let index = newData.findIndex(e => e.provisionName == earnedValByCountry[i].provisionName);
					if (index == -1) {
						newData.push({"provisionName": earnedValByCountry[i].provisionName, "earnedValue" : earnedValByCountry[i].earnedValueTotal});
					} else {
		...
		*/
		...
		HelpService.pageHelp = function () {
			...
			helppagepopup = ngDialog.open({
				template: 'views/help&tutorials/dashboardshelp/earnedvalue/earnvalueprovincehelp.html',
