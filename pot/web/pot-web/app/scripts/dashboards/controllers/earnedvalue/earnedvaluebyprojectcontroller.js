...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("earnedvaluebyproject", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/earnedvalue/earnedvaluebyprojectfilter.html',
				controller: 'EarnedValuByProjectController'
			}
		}
	})
}]).controller("EarnedValuByProjectController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.earnedValueTypes = [{ name: 'Manpower', code: 'projectManpower' }, { name: 'Cost', code: 'projectCost' }];
	...
	$scope.getEarnedValueByProjectDetails = function () {
		if ($scope.earnedValueType.code === "projectManpower") {
			...
			function mergeVal(estAtCompByProject) {
				...
				for (let i=0; i < estAtCompByProject.length; i++) {
					...
					if (index == -1) {
						newData.push({"code": estAtCompByProject[i].code, "earnedValue" : estAtCompByProject[i].earnedManHoursTotal});
					} else {
		...
		function mergeVal(estAtCompByCountry) {
			...
			for (let i=0; i < estAtCompByCountry.length; i++) {
				...
				if (index == -1) {
					newData.push({"code": estAtCompByCountry[i].code, "earnedValue" : estAtCompByCountry[i].earnedValueTotal});
				} else {
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/earnedvalue/earnvalueprojecthelp.html',
