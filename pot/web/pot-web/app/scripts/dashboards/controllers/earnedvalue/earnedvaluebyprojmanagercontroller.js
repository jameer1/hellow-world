...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("earnedvaluebyprojectmananger", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/earnedvalue/earnedvaluebyprojmanagerfilter.html',
				controller: 'EarnedValuByProjManagerController'
			}
		}
	})
}]).controller("EarnedValuByProjManagerController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	"ProjectStatusService", "ProjectSettingsService", "EstimateToCompleteDashboardGroupingService",  function($rootScope, $scope, $q, $state, ngDialog, GenericAlertService, UserEPSProjectService, 
		...
		$scope.earnedValueTypes = [{ name: 'Manpower', code: 'projectManagerManpower' }, { name: 'Cost', code: 'projectManagerCost' }];
		...
		$scope.getEarnedValueByProjectManagerDetails = function () {
			if ($scope.earnedValueType.code === "projectManagerManpower") {
				...
				function mergeVal(earnedHrsByCountry) {
					...
					for (let i=0; i < earnedHrsByCountry.length; i++) {
						let index = newData.findIndex(e => e.name == earnedHrsByCountry[i].name);
						if (index == -1) {
							newData.push({"name": earnedHrsByCountry[i].name, "earnedValue" : earnedHrsByCountry[i].earnedManHoursTotal});
						} else {
			...
			function mergeVal(earnedValByCountry) {
				...
				for (let i=0; i < earnedValByCountry.length; i++) {
					let index = newData.findIndex(e => e.name == earnedValByCountry[i].name);
					if (index == -1) {
						newData.push({"name": earnedValByCountry[i].name, "earnedValue" : earnedValByCountry[i].earnedValueTotal});
					} else {
		...
		*/
		...
		HelpService.pageHelp = function () {
			...
			helppagepopup = ngDialog.open({
				template: 'views/help&tutorials/dashboardshelp/earnedvalue/earnvalueprojmanagerhelp.html',
