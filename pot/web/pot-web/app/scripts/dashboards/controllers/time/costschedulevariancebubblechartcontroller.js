...
app.config(["$stateProvider", function($stateProvider) {
	$stateProvider.state("costandschedulevariancebubblechart", {
		...
		views : {
			'content@' : {
				'templateUrl' : 'views/dashboards/time/costschedulevariancebubblechartfilter.html',
				controller : 'BubbleChartController'
			}
		}
	})
}]).controller("BubbleChartController", ["$rootScope", "$scope", "$q", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
			...
			for (const index in projManPowerValues) {
				for (const index1 in plannedValuesMap) {
					if (plannedValuesMap[index1].projId == projManPowerValues[index].projId) {
						projManPowerValues[index].plannedDirectManHours = plannedValuesMap[index1].directManHours;
						projManPowerValues[index].costVarienceManHours = parseFloat(projManPowerValues[index].earnedManHoursTotal - projManPowerValues[index].actualHrsTotal).toFixed(2);
						projManPowerValues[index].scheduleVarienceDirectManHours = parseFloat(projManPowerValues[index].earnedManHoursTotal - projManPowerValues[index].plannedDirectManHours).toFixed(2);
				...
				$scope.cvSvManHoursBubbleChart.push(projManPowerValues[index]);
			}
			setGraphData($scope.cvSvManHoursBubbleChart)
		}
	}
	...
	function setGraphData(calculatedValues) {
		...
		calculatedValues.map(o => {
			graphData.push([{
				x: o.costVarienceManHours,
				y: o.scheduleVarienceDirectManHours,
				r: o.budget
				
				//x: parseFloat(o.costVarienceManHours).toFixed(2),
				//y: parseFloat(o.scheduleVarienceDirectManHours).toFixed(2),
				//r: Math.sqrt((o.costVarienceManHours) + (o.scheduleVarienceDirectManHours))/5
				//r: o.budget
			}]);
