...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("remainingbudgetbyproject", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/remainingbudget/remainingbudgetbyprojectfilter.html',
				controller: 'RemainingBudgetByProjectController'
			}
		}
	})
}]).controller("RemainingBudgetByProjectController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getRemainingBudgetByProjectDetails = function () {
		if ($scope.budgetType.code === "projectManpower") {
			...
			function mergeVal(estAtCompByProject) {
				...
				for (let i=0; i < estAtCompByProject.length; i++) {
					...
					if (index == -1) {
						newData.push({"code": estAtCompByProject[i].code, "remainingBudget" : estAtCompByProject[i].remainingHrsTotal});
					} else {
						newData[index].budget += estAtCompByProject[i].remainingHrsTotal;
					}
				}
				console.log(newData)
		...
		function mergeVal(estAtCompByCountry) {
			...
			for (let i=0; i < estAtCompByCountry.length; i++) {
				let index = newData.findIndex(e => e.projId == estAtCompByCountry[i].projId);
				if (index == -1) {
					newData.push({"code": estAtCompByCountry[i].code, "remainingBudget" : estAtCompByCountry[i].remainingBudgetTotal});
				} else {
	...
	/*
	$scope.getRemainingBudgetByProjectDetails = function () {
		RemainingBudgetDashboardGroupingService.getRemainingBudgetDashboards($scope.selectedProjIds, $scope.budgetType.code).then(data => {
			setGraphData(data);
		});
	};
	*/
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/remainbudget/remainbudgetbyprojecthelp.html',
