...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("remainingbudgetbycountry", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/remainingbudget/remainingbudgetbycountryfilter.html',
				controller: 'RemainingBudgetByCountryController'
			}
		}
	})
}]).controller("RemainingBudgetByCountryController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	function mergeVal(estAtCompByCountry) {
		...
		for (let i=0; i < estAtCompByCountry.length; i++) {
			...
			if (index == -1) {
				newData.push({"countryName": estAtCompByCountry[i].countryName, "remainingBudget" : estAtCompByCountry[i].remainingBudgetTotal});
			} else {
	...
	*/
	function setGraphData(budgetDetails) {
		...
		$scope.remainingBudgetByCountryTabs.unshift({ 'data': data, 'labels': labels });
		initGraph("pie");
	}
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/remainbudget/remainbudgetbycountryhelp.html',
