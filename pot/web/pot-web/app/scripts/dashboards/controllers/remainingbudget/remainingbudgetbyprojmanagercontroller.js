...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("remainingbudgetbyprojectmanager", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/remainingbudget/remainingbudgetbyprojmanagerfilter.html',
				controller: 'RemainingBudgetByProjManagerController'
			}
		}
	})
}]).controller("RemainingBudgetByProjManagerController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getRemainingBudgetByProjectManagerDetails = function () {
	...
	function mergeVal(estAtCompByCountry) {
		...
		for (let i=0; i < estAtCompByCountry.length; i++) {
			...
			if (index == -1) {
				newData.push({"name": estAtCompByCountry[i].name, "remainingBudget" : estAtCompByCountry[i].remainingBudgetTotal});
			} else {
	...
	/*
	$scope.getRemainingBudgetByProjectManagerDetails = function () {
		RemainingBudgetDashboardGroupingService.getRemainingBudgetDashboards($scope.selectedProjIds, "projectManager").then(data => {
			setGraphData(data);
		});
	...
	*/
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/remainbudget/remainbudgetbyprojmanagerhelp.html',
