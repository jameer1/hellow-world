...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("actualcostbycountry", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/actualcost/actualcostbycountryfilter.html',
				controller: 'ActualCostByCountryController'
			}
		}
	})
}]).controller("ActualCostByCountryController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getActualCostByCountryDetails = function () {
	...
	function mergeVal(budgetByCountry) {
		...
		for (let i=0; i < budgetByCountry.length; i++) {
			...
			if (index == -1) {
				newData.push({"countryName": budgetByCountry[i].countryName, "actualCost" : budgetByCountry[i].actualCostTotal});
			} else {
	...
	/*
	$scope.getActualCostByCountryDetails = function () {
		ActualCostDashboardGroupingService.getActualCostDashboards($scope.selectedProjIds, "country").then(data => {
			setGraphData(data);
		});
	...
	*/
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/actualcost/actcostbycountryhelp.html',
