...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("actualcostbyprojectmanager", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/actualcost/actualcostbyprojmanagerfilter.html',
				controller: 'ActualCostByProjectManagerController'
			}
		}
	})
}]).controller("ActualCostByProjectManagerController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getActualCostByProjectManagerDetails = function () {
	...
	function mergeVal(budgetByCountry) {
		...
		for (let i=0; i < budgetByCountry.length; i++) {
			...
			if (index == -1) {
				newData.push({"name": budgetByCountry[i].name, "actualCost" : budgetByCountry[i].actualCostTotal});
			} else {
				newData[index].actualCost += budgetByCountry[i].actualCostTotal;
			}
		}
		console.log(newData)
	...
	/*
	$scope.getActualCostByProjectManagerDetails = function () {
		ActualCostDashboardGroupingService.getActualCostDashboards($scope.selectedProjIds, "projectManager").then(data => {
			console.log(data)
			setGraphData(data);
		});
	...
	*/
	...
	function setGraphData(budgetDetails) {
		...
		for (const key in budgetDetails) {
			...
			series.push(budgetDetails[key].name);
			data.push(parseFloat(budgetDetails[key].actualCost).toFixed(2));
		}
		$scope.acutlaCostByProjectManagerTabs.unshift({ 'data': data, 'labels': labels, 'series': series });
		initGraph("bar");
	}
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/actualcost/actcostbyprojmanagerhelp.html',
