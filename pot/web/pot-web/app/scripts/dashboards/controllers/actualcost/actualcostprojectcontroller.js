...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("actualcostbyproject", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/actualcost/actualcostbyprojectfilter.html',
				controller: 'ActualCostByProjectController'
			}
		}
	})
}]).controller("ActualCostByProjectController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getActualCostByProjectDetails = function () {
	...
	function mergePlannedValue(progVarienceDataInfo, plannedValuesMap) { 
		...
		if (taskPlannedValue && taskFull) {
			...
			$scope.budgetByProvinceDetails = new Array();
			for (const index_3 in budgetDetails){
				...
				$scope.budgetByProvinceDetails.push(budgetDetails[index_3]);
				
			}
			mergeVal($scope.budgetByProvinceDetails);
		}
	}
	
	function mergeVal(budgetByProvince) {
		console.log("budgetByProvince", budgetByProvince);
		let newData = [];
		for (let i=0; i < budgetByProvince.length; i++) {
			let index = newData.findIndex(e => e.code == budgetByProvince[i].code);
			if (index == -1) {
				newData.push({"code": budgetByProvince[i].code, "actualCost" : budgetByProvince[i].actualCostTotal, "currency" : budgetByProvince[i].currency});
			} else {
				newData[index].actualCost += budgetByProvince[i].actualCostTotal;
			}
		}
		console.log(newData)
	...
	function setGraphData(budgetDetails) {
		...
		for (const key in budgetDetails) {
			...
			labels.push($scope.searchProject.projName.split(',')[index] + " (" + budgetDetails[key].currency + ")");
			series.push($scope.searchProject.projName.split(',')[index] + " " + budgetDetails[key].currency);
			currency.push(budgetDetails[key].currency);
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/actualcost/actcostbyprojecthelp.html',
