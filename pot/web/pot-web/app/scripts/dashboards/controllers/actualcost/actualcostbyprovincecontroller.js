...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("actualcostbyprovince", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/actualcost/actualcostbyprovincefilter.html',
				controller: 'ActualCostByProvinceController'
			}
		}
	})
}]).controller("ActualCostByProvinceController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getActualCostByProvinceDetails = function () {
	...
	function mergeVal(budgetByProvince) {
		...
		for (let i=0; i < budgetByProvince.length; i++) {
			...
			if (index == -1) {
				newData.push({"provisionName": budgetByProvince[i].provisionName, "actualCost" : budgetByProvince[i].actualCostTotal});
			} else {
	...
	/*
	$scope.getActualCostByProvinceDetails = function () {
		ActualCostDashboardGroupingService.getActualCostDashboards($scope.selectedProjIds, "province").then(data => {
			setGraphData(data);
		});
	...
	*/
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/actualcost/actcostbyprovincehelp.html',
