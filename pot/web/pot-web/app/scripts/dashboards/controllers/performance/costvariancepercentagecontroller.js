...
app.config(["$stateProvider", function($stateProvider) {
	$stateProvider.state("costschedulevariancepercentage", {
		...
		views : {
			'content@' : {
				'templateUrl' : 'views/dashboards/performance/costschedulevariancepercentageFilter.html',
				controller : 'CostVariancePercentageController'
			}
		}
	})
}]).controller("CostVariancePercentageController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getVariancePercentageDetails = function() {
	...
	function mergePlannedValue(progVarienceDataInfo, plannedValuesMap) { 
		...
		if (taskPlannedValue && taskFull) {
			...
			for (const index_1 in progVarienceDataInfo) {
				...
				console.log(progVarienceDataInfo[index_1].scheduleVarienceCost);
				console.log(progVarienceDataInfo[index_1].earnedValueTotal)
				console.log(progVarienceDataInfo[index_1].plannedCost);
	...
	function mergeCostManHours(costData, manPowerData) {
		...
		for (const index_3 in costData) {
		  ...
		  for(var csvp of $scope.costSchedVariencePercentage){
			//shedule variance labour hours
			if(csvp.svManHoursExceptional == true){
				    csvp.svlh=Object.is(csvp.schedVarDirManHrsPercentage, NaN) ? "Ahead Schedule" :csvp.schedVarDirManHrsPercentage.toFixed(2) + " "+"Behind Schedule";
			         }
			if(csvp.svManHoursAcceptable == true){
				    csvp.svlh=Object.is(csvp.schedVarDirManHrsPercentage, NaN) ? "Ahead Schedule" :csvp.schedVarDirManHrsPercentage.toFixed(2) + " "+"Behind Schedule";
				     }
			if(csvp.svManHoursWarning == true){
				csvp.svlh=Object.is(csvp.schedVarDirManHrsPercentage, NaN) ? "Behind Schedule" :csvp.schedVarDirManHrsPercentage.toFixed(2) + " "+"Behind Schedule";
			         }
			if(csvp.svManHoursCritical == true){
				csvp.svlh=Object.is(csvp.schedVarDirManHrsPercentage, NaN) ? "Behind Schedule" :csvp.schedVarDirManHrsPercentage.toFixed(2) + " "+"Behind Schedule";
			         }
			//shedule variance labour cost
            if(csvp.svExceptional == true){
				  csvp.svlc=Object.is(csvp.scheduleVarienceCostPercentage, NaN) ? "Ahead Schedule" :csvp.scheduleVarienceCostPercentage.toFixed(2) + " "+"Ahead Schedule";
                     }
            if(csvp.svAcceptable == true){
				  csvp.svlc=Object.is(csvp.scheduleVarienceCostPercentage, NaN) ? "Ahead Schedule" :csvp.scheduleVarienceCostPercentage.toFixed(2) + " "+"Ahead Schedule";
                     }
             if(csvp.svWarning == true){
				  csvp.svlc=Object.is(csvp.scheduleVarienceCostPercentage, NaN) ? "Behind Schedule" :csvp.scheduleVarienceCostPercentage.toFixed(2) + " "+"Behind Schedule";
                     }
             if(csvp.svCritical == true){
				  csvp.svlc=Object.is(csvp.scheduleVarienceCostPercentage, NaN) ? "Behind Schedule" :csvp.scheduleVarienceCostPercentage.toFixed(2) + " "+"Behind Schedule";
                     }
                   //cost variance labour hours
             if(csvp.cvManHoursExceptional == true){
					csvp.cvlh=Object.is(csvp.costVarManHrsPercentage, NaN) ? "Under Budget" :csvp.costVarManHrsPercentage.toFixed(2) +" " + "Under Budget";
                     }
             if(csvp.cvManHoursAcceptable == true){
					csvp.cvlh=Object.is(csvp.costVarManHrsPercentage, NaN) ? "Under Budget" :csvp.costVarManHrsPercentage.toFixed(2) +" " + "Under Budget";
                     }
             if(csvp.cvManHoursWarning == true){
					csvp.cvlh=Object.is(csvp.costVarManHrsPercentage, NaN) ? "Under Budget" :csvp.costVarManHrsPercentage.toFixed(2) + " " +"Over Budget";
                     }
             if(csvp.cvManHoursCritical == true){
					csvp.cvlh=Object.is(csvp.costVarManHrsPercentage, NaN) ? "Under Budget" :csvp.costVarManHrsPercentage.toFixed(2) + " " +"Over Budget";
                     }
                  //cost variance cost
              if(csvp.cvExceptional == true){
				csvp.cvc=Object.is(csvp.costVarienceCostPercentage, NaN) ? "Under Budget" :csvp.costVarienceCostPercentage.toFixed(2) + "Under Budget";
                     }   
			  if(csvp.cvAcceptable == true){
				
				csvp.cvc=Object.is(csvp.costVarienceCostPercentage, NaN) ? "Under Budget" :csvp.costVarienceCostPercentage.toFixed(2) + "Under Budget";
				
			         }
			  if(csvp.cvWarning == true){
				csvp.cvc=Object.is(csvp.costVarienceCostPercentage, NaN) ? "Over Budget" :csvp.costVarienceCostPercentage.toFixed(2) +" " + "Over Budget";
			         }
			  if(csvp.cvCritical == true){
					csvp.cvc=Object.is(csvp.costVarienceCostPercentage, NaN) ? "Over Budget" :csvp.costVarienceCostPercentage.toFixed(2) + " "+"Over Budget";
			         }     
		}   
		$scope.gridOptions.data = angular.copy($scope.costSchedVariencePercentage);
		console.log("costSchedVariencePercentage", $scope.costSchedVariencePercentage);
	}
		...
		for (const costValue of projCostStmtDtls) {
			...
			for (let i=0; i < projCostValues.length; i++) {
				...
				if (projCostValues[i].projId == costValue.projId) {
					...
					//CV= Earned Hours -  Actual  Hours
					//projCostValues[i].costVarienceLabourHours = (projCostValues[i].earnedValueTotal - projCostValues[i].actualCostTotal);
			...
			function (newValue, oldValue) {
				if (newValue) {
					let columnDefs = [
						...
						{ field: 'svlh', displayName: "Schedule Variance - Labour Hours", headerTooltip: "Schedule Variance - Labour Hours",
						...
						{ field: 'svlc', displayName: "Schedule Variance - Cost", headerTooltip: "Schedule Variance - Cost",
						...
						{ field: 'cvlh', displayName: "Cost Variance - Labour Hours", headerTooltip: "Cost Variance - Labour Hours",
						...
						{ field: 'cvc', displayName: "Cost Variance - Cost", headerTooltip: "Cost Variance - Cost", 
					...
					let data = [];
					$scope.gridOptions = ngGridService.initGrid($scope, columnDefs, data, "Dashboard_Performance_ScheduleVariancePercentage");
