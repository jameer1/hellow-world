...
app.config(["$stateProvider", function($stateProvider) {
	$stateProvider.state("costandscheduleperformance", {
		...
		views : {
			'content@' : {
				'templateUrl' : 'views/dashboards/performance/costscheduleandperformancefilter.html',
				controller : 'CostScheduleAndPerformanceController'
			}
		}
	})
}]).controller("CostScheduleAndPerformanceController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getCostScheduleDetails = function() {
	...
	function mergePlannedValue(progVarienceDataInfo, plannedValuesMap) { 
		if (taskPlannedValue && taskFull) {
			...
			for(var csp of $scope.costSchedPerformanceData){
				//Cost variance
				if(csp.cvExceptional == true){
					csp.CostVariance=csp.costVarience.toFixed(2)+"Under Budget";
				}
				if(csp.cvAcceptable == true){
					csp.CostVariance=csp.costVarience.toFixed(2) +"Under Budget";
				}
				if(csp.cvWarning == true){
					csp.CostVariance=csp.costVarience.toFixed(2) +"Over Budget";
				}
				if(csp.cvCritical == true){
					csp.CostVariance=csp.costVarience.toFixed(2) +"Over Budget";
				}
				//Shedule variance
				if(csp.svExceptional == true){
					csp.SheduleVariance=csp.scheduleVarienceCost.toFixed(2) +"Ahead of Schedule";
				}
				if(csp.svAcceptable == true){
					csp.SheduleVariance=csp.scheduleVarienceCost.toFixed(2) +"Ahead of Schedule";
				}
				if(csp.svWarning == true){
					csp.SheduleVariance=csp.scheduleVarienceCost.toFixed(2)+"Behind Schedule";
				}
				if(csp.svCritical == true){
					csp.SheduleVariance=csp.scheduleVarienceCost.toFixed(2) +"Behind Schedule";
				}
            	//SPI
            	if(csp.spiExceptional == true){
	                csp.spii=csp.schedulePerformanceIndex.toFixed(2) +"Project in Acceleration";
                 }
                 if(csp.spiAcceptable == true){
	                csp.spii=csp.schedulePerformanceIndex.toFixed(2) +"On Schedule";
                 }
                 if(csp.spiWarning == true){
					csp.spii=csp.schedulePerformanceIndex.toFixed(2)+"Project is Delayed";
                 }
                 if(csp.spiCritical == true){
					csp.spii=csp.schedulePerformanceIndex.toFixed(2) +"Project is Delayed";
                 }
				//CPI
				if(csp.cpiExceptional == true){
					csp.cpii=csp.costPerformanceIndex.toFixed(2) +"Project in Acceleration";
				}
				if(csp.cpiAcceptable == true){
					csp.cpii=csp.costPerformanceIndex.toFixed(2) +"Project in Acceleration";
				}
				if(csp.cpiWarning == true){
					csp.cpii=csp.costPerformanceIndex.toFixed(2) +"Project is Delayed";
				}
				if(csp.cpiCritical == true){
					csp.cpii=csp.costPerformanceIndex.toFixed(2) +"Project is Delayed";
				}
				//TCPI
				if(csp.TcpiExceptional == true){
					if(csp.tcpii==null ||csp.tcpii==undefined || csp.tcpii==NaN){
						csp.tcpii="";
					}
					csp.tcpii=csp.toCompleteCostPerformanceIndex.toFixed(2) +"No need for budget revision";
				}
				if(csp.TcpiAcceptable == true){
					if(csp.tcpii==null ||csp.tcpii==undefined || csp.tcpii==NaN){
						csp.tcpii="";
					}
					csp.tcpii+=csp.toCompleteCostPerformanceIndex.toFixed(2) +"No need for budget revision";
				}
				if(csp.TcpiWarning == true){
					if(csp.tcpii==null ||csp.tcpii==undefined || csp.tcpii==NaN){
						csp.tcpii="";
					}
					csp.tcpii+=csp.toCompleteCostPerformanceIndex.toFixed(2) +"Need for budget revision";
				}
				if(csp.TcpiCritical == true){
					if(csp.tcpii==null ||csp.tcpii==undefined || csp.tcpii==NaN){
						csp.tcpii="";
					}
					csp.tcpii+=csp.toCompleteCostPerformanceIndex.toFixed(2)+"Need for budget revision";
			...
			console.log($scope.costSchedPerformanceData,"costSchedPerformanceData")
			...
			function (newValue, oldValue) {
				if (newValue) {
					let columnDefs = [
						...
						{ name: 'CostVariance', displayName: "Cost Variance", headerTooltip: "Cost Variance",
						...
						{ name: 'SheduleVariance', displayName: "Schedule Variance", headerTooltip: "Schedule Variance",
						...
						{ name: 'spii', displayName: "SPI", headerTooltip: "SPI",
						...
						{ name: 'cpii', displayName: "CPI", headerTooltip: "CPI",
						...
						{ name: 'tcpii', displayName: "TCPI", headerTooltip: "TCPI",
					...
					let data = [];
					$scope.gridOptions = ngGridService.initGrid($scope, columnDefs, data, "Dashboard_Performance_Cost Schedule & Performance");
				}
			});
