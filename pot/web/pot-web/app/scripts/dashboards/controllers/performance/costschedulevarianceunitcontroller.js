...
app.config(["$stateProvider", function($stateProvider) {
	$stateProvider.state("costandschedulevarianceunits", {
		...
		views : {
			'content@' : {
				'templateUrl' : 'views/dashboards/performance/costschedulevarianceunitsfilter.html',
				controller : 'CostScheduleVarianceUnitsController'
			}
		}
	})
}]).controller("CostScheduleVarianceUnitsController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getCostScheduleUnitsDetails = function() {
		...
		ProjectSettingsService.getProjPlannedValue(req).then(function (data) {
			...
			$scope.gridOptions.data = angular.copy($scope.plannedValuesMap);
	...
	function mergePlannedValue(progVarienceDataInfo, plannedValuesMap) { 
		...
		if (taskPlannedValue && taskFull) {
			...
			console.log("FINAL", $scope.costSchedVarienceData);
				$scope.gridOptions.data = angular.copy($scope.costSchedVarienceData);
		}
	...
	function mergeCostManHours(costData, manPowerData) {
		...
		for (const index_3 in costData) {
			...
			$scope.costSchedVarienceInfo.push(costData[index_3]);
				...
				for(var csv of $scope.costSchedVarienceInfo){
				//schedule variance labour hours
				if(csv.svManHoursExceptional == true){
					csv.Svlh=csv.scheduleVarienceDirectManHours.toFixed(2)+"Ahead of  Schedule";
				}
				if(csv.svManHoursAcceptable == true){
					csv.Svlh=csv.scheduleVarienceDirectManHours.toFixed(2) +"Ahead of Schedule";
				}
				if(csv.svManHoursWarning == true){
					csv.Svlh=csv.scheduleVarienceDirectManHours.toFixed(2) +"Behind Schedule";
				}
				if(csv.svManHoursCritical == true){
					csv.Svlh=csv.scheduleVarienceDirectManHours.toFixed(2)+"Behind Schedule";
				}
				//shedule variance cost
				if(csv.svExceptional == true){
					csv.svc=csv.scheduleVarienceCost.toFixed(2)+"Ahead of Schedule";
				}
				if(csv.svAcceptable == true){
					csv.svc=csv.scheduleVarienceCost.toFixed(2)+"Ahead of Schedule";
				}
				if(csv.svWarning == true){
					csv.svc=csv.scheduleVarienceCost.toFixed(2) +"Behind Schedule";
				}
				if(csv.svCritical == true){
					csv.svc=csv.scheduleVarienceCost.toFixed(2) +"Behind Schedule";
				}
			//cost variance labour hours                      
               if(csv.cvManHoursExceptional == true){
					csv.cvlh=csv.costVarienceManHours.toFixed(2) +"Under Budget";
             	}
            	 if(csv.cvManHoursAcceptable == true){
					csv.cvlh=csv.costVarienceManHours.toFixed(2)+"Under Budget";
           		}
            	if(csv.cvManHoursWarning == true){
					csv.cvlh=csv.costVarienceManHours.toFixed(2)+"Over Budget";
            	}
            	if(csv.cvManHoursCritical == true){
					csv.cvlh=csv.costVarienceManHours.toFixed(2)+"Over Budget";
				}
				//cost variance cost             	
             	if(csv.cvExceptional == true){
					csv.cvc=csv.costVarience.toFixed(2)+"Under Budget";
 				}
 				if(csv.cvAcceptable == true){
					csv.cvc=csv.costVarience.toFixed(2) +"Under Budget";
				}
				if(csv.cvWarning == true){
					csv.cvc=csv.costVarience.toFixed(2) +"Over Budget";
				}
				if(csv.cvCritical == true){
					csv.cvc=csv.costVarience.toFixed(2) +"Over Budget";
				}
			}
		$scope.gridOptions.data = angular.copy($scope.costSchedVarienceInfo);
		console.log("costSchedVarienceInfo", $scope.costSchedVarienceInfo);
	}
			...
			function (newValue, oldValue) {
				if (newValue) {
					let columnDefs = [
						...
						{ field: 'svc', displayName: "Schedule Variance - Cost", headerTooltip: "Schedule Variance - Cost",
					...
					let data = [];
					$scope.gridOptions = ngGridService.initGrid($scope, columnDefs, data, "Dashboard_Performance_ScheduleVarianceUnit");
