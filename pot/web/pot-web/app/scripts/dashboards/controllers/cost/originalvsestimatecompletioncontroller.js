...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("originalvsestimateatcompletioncost", {
		...
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/cost/originalvsestimatecompletioncostfilter.html',
				controller: 'OriginalVsEstimateController'
			}
		}
	})
}]).controller("OriginalVsEstimateController", ["$rootScope", "uiGridGroupingConstants", "uiGridConstants", "stylesService" ,"ngGridService", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.getOriginalEstimateDetails = function () {
	...
	function mergePlannedValue(progVarienceDataInfo, plannedValuesMap) { 
		if (taskPlannedValue && taskFull) {
			...
			for(let i=0;i<$scope.costSchedPerformanceData.length;i++){
					if($scope.costSchedPerformanceData[i].estimateType == "Remaining Units"){
					if($scope.costSchedPerformanceData[i].revisedBudgetTotal > 0){
					($scope.costSchedPerformanceData[i]).estimateCompletion = ($scope.costSchedPerformanceData[i].revisedBudgetTotal)-($scope.costSchedPerformanceData[i].actualCostTotal);
				}else{
					($scope.costSchedPerformanceData[i]).estimateCompletion = ($scope.costSchedPerformanceData[i].originalBudgetTotal)-($scope.costSchedPerformanceData[i].actualCostTotal);
				}
				}else if($scope.costSchedPerformanceData[i].estimateType == "BAC-EV"){
					if($scope.costSchedPerformanceData[i].revisedBudgetTotal > 0){
					($scope.costSchedPerformanceData[i]).estimateCompletion = ($scope.costSchedPerformanceData[i].revisedBudgetTotal)-($scope.costSchedPerformanceData[i].earnedValueTotal);
				}else{
					($scope.costSchedPerformanceData[i]).estimateCompletion = ($scope.costSchedPerformanceData[i].originalBudgetTotal)-($scope.costSchedPerformanceData[i].earnedValueTotal);
				}
				}else if($scope.costSchedPerformanceData[i].estimateType == "(BAC-EV)/CPI"){
					if($scope.costSchedPerformanceData[i].revisedBudgetTotal > 0){
					($scope.costSchedPerformanceData[i]).estimateCompletion = ($scope.costSchedPerformanceData[i].revisedBudgetTotal-$scope.costSchedPerformanceData[i].earnedValueTotal)/(($scope.costSchedPerformanceData[i].earnedValueTotal/$scope.costSchedPerformanceData[i].actualCostTotal));
				}else{
					($scope.costSchedPerformanceData[i]).estimateCompletion = ($scope.costSchedPerformanceData[i].originalBudgetTotal-$scope.costSchedPerformanceData[i].earnedValueTotal)/(($scope.costSchedPerformanceData[i].earnedValueTotal/$scope.costSchedPerformanceData[i].actualCostTotal));
				}
				}else if($scope.costSchedPerformanceData[i].estimateType == "(BAC-EV)/(CPI*SPI)"){
					if($scope.costSchedPerformanceData[i].revisedBudgetTotal > 0){
					($scope.costSchedPerformanceData[i]).estimateCompletion = ($scope.costSchedPerformanceData[i].revisedBudgetTotal-$scope.costSchedPerformanceData[i].earnedValueTotal)/(($scope.costSchedPerformanceData[i].earnedValueTotal/$scope.costSchedPerformanceData[i].actualCostTotal)*($scope.costSchedPerformanceData[i].earnedValueTotal/$scope.costSchedPerformanceData[i].plannedCost));
				}else{
					($scope.costSchedPerformanceData[i]).estimateCompletion = ($scope.costSchedPerformanceData[i].originalBudgetTotal-$scope.costSchedPerformanceData[i].earnedValueTotal)/(($scope.costSchedPerformanceData[i].earnedValueTotal/$scope.costSchedPerformanceData[i].actualCostTotal)*($scope.costSchedPerformanceData[i].earnedValueTotal/$scope.costSchedPerformanceData[i].plannedCost));
				}
				}else if($scope.costSchedPerformanceData[i].estimateType == "New Estimate"){
					($scope.costSchedPerformanceData[i]).estimateCompletion = $scope.costSchedPerformanceData[i].estCompletion;
				}
				}
	
			console.log("FINAL", $scope.costSchedPerformanceData);
			$scope.gridOptions.data = angular.copy($scope.costSchedPerformanceData);
		}
	...
	$scope.$watch(function () { return stylesService.finishedStyling; }, function (newValue, oldValue) {
		if (newValue) {
			let columnDefs = [
				...
				{ name: 'actualCostTotal',displayName:'Actual To Date',headerTooltip: "Actual To Date", footerCellFilter:'number:2', cellFilter: 'number: 2', groupingShowAggregationMenu: false, aggregationType: uiGridConstants.aggregationTypes.sum, treeAggregationType: uiGridGroupingConstants.aggregation.SUM,
				customTreeAggregationFinalizerFn: function (aggregation) {
					aggregation.rendered = aggregation.value;
				}},	
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/costhelp/orgvsestcomphelp.html',
