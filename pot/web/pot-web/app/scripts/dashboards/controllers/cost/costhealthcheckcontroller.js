...
app.config(["$stateProvider", function ($stateProvider) {
	$stateProvider.state("costhealthcheck", {
		url: '/costhealthcheck',
		data: {
			dashboards: []
		},
		views: {
			'content@': {
				'templateUrl': 'views/dashboards/cost/costhealthcheckfilter.html',
				controller: 'CostHealthCheckController'
			}
		}
	})
}]).controller("CostHealthCheckController", ["$rootScope", "$scope", "$q", "$state", "ngDialog", "GenericAlertService", "UserEPSProjectService", 
	...
	$scope.costHealthTabs = new Array();
	$scope.getCostHealthCheckDetails = function () {
	...
	function mergePlannedValue(progVarienceDataInfo, plannedValuesMap) { 
		...
		if (taskPlannedValue && taskFull) {
			...
			for (const index_1 in progVarienceDataInfo) {
				...
				console.log("EARNED VALUE", progVarienceDataInfo[index_1].earnedValueTotal);
				console.log("PLANNED VALUE", progVarienceDataInfo[index_1].plannedCost);
			...
			for (const index_4 in manPowerData) {
				if(manPowerData[index_4].projId == costData[index_3].projId) {
					...
					if(costData[index_3].tcpiManHrs >= costData[index_3].exTcpiUpperLimit
						...
						costData[index_3].TcpiLabourExceptional = true;
					}
					if(costData[index_3].tcpiManHrs >= costData[index_3].acTcpiUpperLimit
						...
						costData[index_3].TcpiLabourAcceptable = true;
					}
					if(costData[index_3].tcpiManHrs >= costData[index_3].waTcpiUpperLimit
						...
						costData[index_3].TcpiLabourWarning = true;
					}
					if(costData[index_3].tcpiManHrs >= costData[index_3].crTcpiLowerLimit) {
						costData[index_3].TcpiLabourCritical = true;
					}
				}
			}
...
//SPI
 ...
 //TCPI
		...
		console.log("costHealthCheck", $scope.costHealthCheck);
		$scope.gridOptions.data = angular.copy($scope.costHealthCheck);
	}
		...
		for (const value of projManpowerDetails) {
			...
			for (let i=0; i < projManPowerValues.length; i++) {
				...
				if (projManPowerValues[i].projId == value.projId) {
					...
					console.log("DIRECT BUDGET VALUE", value.projEmpCategory);
	...
	HelpService.pageHelp = function () {
		...
		helppagepopup = ngDialog.open({
			template: 'views/help&tutorials/dashboardshelp/costhelp/costhealthcheckhelp.html',
	...
	function (newValue, oldValue) {
				if (newValue) {
					...
					let data = [];
					$scope.gridOptions = ngGridService.initGrid($scope, columnDefs, data, "Schedules_ResourcesAssignmentDataTable");
	                 $scope.getDatasets();				
				}
			});
