'use strict';
app.factory('PlantTransfersDetailsFactory', ["ngDialog", "$q", "$filter", "$timeout", "$rootScope", "GenericAlertService", "ProjectCrewPopupService", "PlantTransferService", "PlantRegisterService", "UserEPSProjectService", function(ngDialog, $q, $filter, $timeout, $rootScope, GenericAlertService, ProjectCrewPopupService, PlantTransferService, PlantRegisterService, UserEPSProjectService) {
	var getPlantTransferDetailsPopup;
	var service = {};
	service.getPlantTransferDetailsPopup = function() {
		var deferred = $q.defer();
		getPlantTransferDetailsPopup = ngDialog.open({
			template : 'views/projresources/planttransfer/plantregdetailspopup.html',
			...
			controller : [ '$scope', function($scope) {
				...
				$scope.getPlantFromProjects = function() {
					...
					PlantRegisterService.getPlantRegistersOnLoad($scope.getPlantRegisterReq).then(function(data) {
						...
						GenericAlertService.alertMessage("Error occured while getting Registered Plant Details", "Error");
					});
				}
				$scope.plantRegisterPopUp = function(projPlantRegisterTO) {
					var returnProjPlantRegisterTO = {
						"plantRegisterDtlTO" : projPlantRegisterTO
					};
					deferred.resolve(returnProjPlantRegisterTO);
					$scope.closeThisDialog();

				}
				$scope.getPlantToProjects = function(plantPopUp) {
				...
				}, $scope.getApproversList = function(plantPopUp, projLabelKeyTO) {
					$scope.projId = $scope.searchPlantProject.projId;
					var ApproverSerivcePopup = [];
					ApproverSerivcePopup = ProjectCrewPopupService.approverDetailsList($scope.projId);
				...
				$scope.addCreateRequestRowData = function() {
					if (actionType === 'Add') {
						...
						$scope.addPlantTransfers = angular.copy(editCreateRequestDetails);
						editCreateRequestDetails = [];
					}
				}

				$scope.plantTranferPopUpRowSelect = function(plantPopUp) {
					if (plantPopUp.select) {
						selectedPlant.push(plantPopUp);
					} else {
						selectedPlant.pop(plantPopUp);
					}
				...
				$scope.savePlantCreate = function() {
					...
					PlantTransferService.savePlantCreateTransfer(savePlantTransferReq).then(function(data) {
						GenericAlertService.alertMessage('Employee Registration Detail(s) is/are ' + data.message, data.status);
					}, function(error) {
						GenericAlertService.alertMessage('Employee Registration Detail(s) is/are Failed to Save ', "Warning");
					});
					ngDialog.close();
				}
			} ]
