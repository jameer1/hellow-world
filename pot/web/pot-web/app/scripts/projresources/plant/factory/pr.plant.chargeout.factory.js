'use strict';
app.factory('PlantChargeFactory', ["ngDialog", "$q", "$filter", "blockUI", "$timeout", "$rootScope", "EmpCostCodeFactory", "GenericAlertService", "PlantRegisterService", function(ngDialog, $q, $filter, blockUI,$timeout, $rootScope, EmpCostCodeFactory, GenericAlertService, PlantRegisterService) {
	var plantChargePopUp;
	var service = {};
	service.plantChargeOnLoad = function(itemData) {
		var deferred = $q.defer();
		plantChargePopUp = ngDialog.open({
			template : 'views/projresources/projplantreg/plantcharge/plantregchargepopup.html',
			...
			controller : [ '$scope', function($scope) {
				...
				$scope.savePlantChargeOutRates = function() {
					...
					blockUI.start();
					PlantRegisterService.savePlantChargeOutRates(req).then(function(data) {
						blockUI.stop();
						// var succMsg = GenericAlertService.alertMessageModal('Plant ChargeOut Detail(s) is/are  ' + data.message, data.status);
						var succMsg = GenericAlertService.alertMessageModal('Plant Charge out Rate(s) saved successfully',"Info");
						...
						if (error.message != null && error.status != null) {
							...
							GenericAlertService.alertMessage('Error occured while saving the project chargeout details', "Error");
						}
					});
				},
