...
app.factory('ProjectSettingCostItemFactory', ["ngDialog", "$q", "$filter", "$timeout", "$rootScope", "blockUI", "ProjectSettingsService", "GenericAlertService", function(ngDialog, $q, $filter, $timeout, $rootScope,blockUI,ProjectSettingsService, GenericAlertService) {
	...
	costCodeItemService.getCostItemDetails = function(projId) {
		...
		costStmtPopUp = ngDialog.open({
			...
			controller : [ '$scope', function($scope) {
				...
				ProjectSettingsService.getCostDetails(costReq).then(function(data) {
				...
				$scope.popUpRowSelect = function(tab) {
					...
					blockUI.start();
					ProjectSettingsService.saveProjCostCodes(saveCostStmtReq).then(function(data) {
						blockUI.stop();
