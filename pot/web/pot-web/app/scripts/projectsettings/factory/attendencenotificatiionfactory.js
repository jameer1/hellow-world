'use strict';
app.factory('ProjectAttendencePopupService', ["ngDialog", "$q", "$filter", "$timeout", "NotificationService", "$rootScope", "ProjectSettingsService", "GenericAlertService", function(ngDialog, $q, $filter, $timeout, NotificationService,$rootScope, ProjectSettingsService, GenericAlertService) {
	var crewServicePopup = [];
	var crewService = {};
	crewService.crewDetailsList = function(projCrewTOs) {
		var deferred = $q.defer();
		crewServicePopup = ngDialog.open({
			template : 'views/projectsettings/attendencenotification.html',
		...
		var deferred = $q.defer();
		var prjcrewListPromise =NotificationService.getAttendanceNotifications(req)
		prjcrewListPromise.then(function(data) {
			var projCrewTOs = [], filteredCrewList = [];
			projCrewTOs = data.attendenceNotificationsTOs;

			/*Notification type filter starts*/
			if( req.hasOwnProperty('type') && (req.type == "EMP" || req.type == "PLANT") ){
				filteredCrewList = projCrewTOs.filter( (list) => {
					return list.moduleCode == req.type + "-Notification";
				});
			}else{
				filteredCrewList = projCrewTOs;
			}
			/*Notification type filter ends*/

			var crewSerivcePopup = crewService.crewDetailsList(filteredCrewList);
