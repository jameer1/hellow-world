...
app.factory('ProjectSettingSOWItemFactory', ["ngDialog", "$q", "$filter", "$timeout", "$rootScope", "blockUI", "ProjectSettingsService", "GenericAlertService", function(ngDialog, $q, $filter, $timeout, $rootScope,blockUI,ProjectSettingsService, GenericAlertService) {
	var SOWItemService = {};
	var projSOWPopUp = [];
	SOWItemService.getSOWItemDetails = function(projId) {
		...
		projSOWPopUp = ngDialog.open({
			...
			controller : [ '$scope', function($scope) {
				...
				ProjectSettingsService.getProjSOWDetails(sowReq).then(function(data) {
				...
				$scope.saveProjSOWItems = function() {
					...
					blockUI.start();
					ProjectSettingsService.saveProjSOWItems(saveProgMesureReq).then(function(data) {
						blockUI.stop();
