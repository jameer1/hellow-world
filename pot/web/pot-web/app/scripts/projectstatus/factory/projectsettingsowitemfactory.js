...
app.factory('ProjectSettingSOWItemFactory', ["ngDialog", "$q", "$filter", "$timeout", "$rootScope", "blockUI", "ProjectStatusService", "GenericAlertService", function(ngDialog, $q, $filter, $timeout, $rootScope,blockUI,ProjectStatusService, GenericAlertService) {
	var SOWItemService = {};
	var projSOWPopUp = [];
	SOWItemService.getSOWItemDetails = function(projId) {
		var deferred = $q.defer();
		projSOWPopUp = ngDialog.open({
			template : 'views/projectstatus/sowitempopup.html',
			...
			controller : [ '$scope', function($scope) {
				...
				ProjectStatusService.getProjSOWDetails(sowReq).then(function(data) {
				...
				$scope.saveProjSOWItems = function() {
					...
					blockUI.start();
					ProjectStatusService.saveProjSOWItems(saveProgMesureReq).then(function(data) {
						blockUI.stop();
