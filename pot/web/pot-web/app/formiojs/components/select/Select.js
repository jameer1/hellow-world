...
var SelectComponent = /*#__PURE__*/function (_Field) {
  ...
  _createClass(SelectComponent, [{
    ...
    value: function itemTemplate(data) {
      ...
      if (data.data) {
        data.data = this.isEntireObjectDisplay() && _lodash.default.isObject(data.data) ? JSON.stringify(data.data) : data.data;
      }
    ...
    value: function addOption(value, label) {
      ...
      var option = {
        id: id || (0, _utils.getRandomComponentId)(),
        value: _lodash.default.isObject(value) && this.isEntireObjectDisplay() ? this.normalizeSingleValue(value) : _lodash.default.isObject(value) ? value : _lodash.default.isNull(value) ? this.emptyValue : String(this.normalizeSingleValue(value)),
        label: label
      };
    ...
    key: "refresh",
    value: function refresh(value, _ref) {
    ...
    value: function getNormalizedValues() {
      ...
      return this.component.data.values.map(function (value) {
        return {
          label: value.label,
          value: String(_this8.normalizeSingleValue(value.value))
        };
      });
    }
  }, {
    ...
    value: function asString(value) {
      ...
      var convertToString = function convertToString(data, valueProperty) {
        ...
        if (_this9.isBooleanOrNumber(data)) {
          data = data.toString();
        }

        if (Array.isArray(data) && data.some(function (item) {
          return _this9.isBooleanOrNumber(item);
        })) {
          data = data.map(function (item) {
            if (_this9.isBooleanOrNumber(item)) {
              item = item.toString();
            }
          });
        }
      ...
      if (['values', 'custom'].includes(this.component.dataSrc)) {
        var _ref2 = this.component.dataSrc === 'values' ? {
          ...
          valueProperty: this.valueProperty
            ...
            items = _ref2.items,
            valueProperty = _ref2.valueProperty;
      ...
      if (Array.isArray(value)) {
        ...
        value.forEach(function (item) {
          return _items.push(_this9.itemTemplate(item));
        });
      ...
      if (this.component.data && this.component.data.headers) {
        try {
          _lodash.default.each(this.component.data.headers, function (header) {
            if (header.key) {
              headers.set(header.key, _this10.interpolate(header.value));
            }
          });
        } catch (err) {
          console.warn(err.message);
