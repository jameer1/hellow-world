...
var WebformBuilder = /*#__PURE__*/function (_Component) {
  ...
  _createClass(WebformBuilder, [{
    ...
    value: function initDragula() {
      ...
      console.log("inside the drop event");
    ...
    value: function onDrop(element, target, source, sibling) {
      ...
      return rebuild.then(function () {
        _this6.emit('addComponent', info, parent, path, index, isNew && !_this6.options.noNewEdit && !info.noNewEdit);

        if (!isNew || _this6.options.noNewEdit || info.noNewEdit) {
    ...
    value: function saveComponent(component, parent, isNew, original) {
      ...
      if (index !== -1) {
        ...
        return rebuild.then(function () {
          ...
          _this9.emit('saveComponent', schema, originalComp, parentComponent.schema, path, index, isNew, originalComponentSchema);
    ...
    value: function editComponent(component, parent, isNew, isJsonEdit, original) {
        ...
        if (event.changed) {
          ...
          if (event.changed.component && ['label', 'title'].includes(event.changed.component.key)) {
            ...
            if (isNew) {
              ...
              if (_this10.form) {
                var formComponents = _this10.findNamespaceRoot(parent.formioComponent.component); // excluding component which key uniqueness is to be checked to prevent the comparing of the same keys
      ...
      var dialogClose = function dialogClose() {
        _this10.editForm.destroy(true);

        if (_this10.preview) {
          _this10.preview.destroy(true);
