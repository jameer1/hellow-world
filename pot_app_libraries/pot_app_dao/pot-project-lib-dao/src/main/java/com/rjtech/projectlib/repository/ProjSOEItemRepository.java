...
public interface ProjSOEItemRepository extends ProjLibBaseRepository<ProjSOEItemEntity, Long> {
    ...
    @Query("SELECT SOE FROM ProjSOEItemEntity SOE WHERE  SOE.projMstrEntity.projectId=:projId AND SOE.id=:soeId AND SOE.status=:status  ORDER BY  SOE.code")
    public List<ProjSOEItemEntity> findSOEDetailsById(@Param("projId") Long projId, @Param("soeId") Long soeId,
            @Param("status") Integer status);

    @Modifying
    @Query("UPDATE ProjSOEItemEntity SOE  SET SOE.status=:status  where SOE.id in :soeIds or SOE.projSOEItemEntity.id in :soeIds")
    void deactivateSOEDetails(@Param("soeIds") List<Long> soeIds, @Param("status") Integer status);

    @Query("SELECT SOE FROM ProjSOEItemEntity SOE WHERE  SOE.projMstrEntity.projectId=:projId ORDER BY  SOE.code")
    public List<ProjSOEItemEntity> findAllSOEDetails(@Param("projId") Long projId);
    
    @Modifying
    @Query("UPDATE ProjSOEItemEntity SOE SET SOE.soeItemStatus=:soeItemStatus,SOE.internalApproverUserId.userId=:internalApprovalUserId,SOE.internalApprovalStatus=:internalApprovalStatus,SOE.originatorUserId.userId=:originatorUserId where SOE.id=:soeId")
    ...
    @Modifying
    @Query("UPDATE ProjSOEItemEntity SOE SET SOE.soeItemStatus=:soeItemStatus,SOE.isExternalApprovalRequired=:isExternalApprovalRequired,SOE.internalApprovalStatus=:internalApprovalStatus,SOE.internalApproverComments=:internalApproverComments,SOE.status=:status where SOE.id=:soeId")
    ...
    @Modifying
    @Query("UPDATE ProjSOEItemEntity SOE SET SOE.soeItemStatus=:soeItemStatus,SOE.externalApproverUserId.userId=:externalApprovalUserId,SOE.externalApprovalStatus=:externalApprovalStatus where SOE.id=:soeId")
    ...
    @Modifying
    @Query("UPDATE ProjSOEItemEntity SOE SET SOE.soeItemStatus=:soeItemStatus,SOE.externalApprovalStatus=:externalApprovalStatus,SOE.externalApproverComments=:externalApproverComments,SOE.status=:status where SOE.id=:soeId")
    ...
    @Modifying
    @Query("UPDATE ProjSOEItemEntity SOE SET SOE.soeItemStatus=:soeItemStatus,SOE.isItemReturned=:isItemReturned where SOE.id=:soeId")
    ...
    @Modifying
    @Query("UPDATE ProjSOEItemEntity SOE SET SOE.soeItemStatus=:soeItemStatus where SOE.id=:soeId")
    public void updateApprovalStatusBySoeId( @Param("soeId") Long soeId, @Param("soeItemStatus") String soeItemStatus );
    
    @Query("SELECT SOE FROM ProjSOEItemEntity SOE WHERE SOE.projMstrEntity.projectId=:projId AND SOE.id in :soeIds AND SOE.status=:status ORDER BY SOE.code")
    ...
    @Query("SELECT SOE FROM ProjSOEItemEntity SOE WHERE  SOE.projMstrEntity.projectId=:projId AND SOE.id=:soeId AND SOE.status NOT IN (2,3) ORDER BY SOE.code")
    public List<ProjSOEItemEntity> findSOEItemsById( @Param("projId") Long projId, @Param("soeId") Long soeId );
    
    @Query("SELECT SOE FROM ProjSOEItemEntity SOE WHERE SOE.id in :parentSoeIds")
