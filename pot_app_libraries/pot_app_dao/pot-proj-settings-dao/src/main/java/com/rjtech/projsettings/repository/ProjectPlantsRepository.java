 ...
 * 
 * import com.rjtech.centrallib.model.PlantMstrEntity; import
 * com.rjtech.common.model.ClientRegEntity; import
 * com.rjtech.eps.model.ProjMstrEntity; import
 * com.rjtech.projsettings.model.ProjectPlantsDtlEntity;
 * 
 * public interface ProjectPlantsRepository extends
 * ProjSettingsBaseRepository<ProjectPlantsDtlEntity, Long> {
 * 
 * @Query("SELECT PJP FROM ProjectPlantsDtlEntity PJP WHERE  PJP.projMstrEntity.projectId=:projId AND PJP.status=:status"
 * ) public List<ProjectPlantsDtlEntity> findProjectPlants(@Param("projId") Long
 ...
 * ) public List<ProjectPlantsDtlEntity> findProjectPlants( @Param("projId")
 * Long projId );
 * 
 ...
 * + "PJP.plantMstrEntity.id") public List<Object[]>
 * findProjectPlantsStatus(@Param("projId") Long projId, @Param("status")
 * Integer status);
 * 
 * @Query("SELECT PCM.id," + "    SUM(WDPS.usedTotal + WDPS.idleTotal) " +
 * "from PlantMstrEntity PCM " +
 ...
 * "        OR (WDPS.apprStatus = 'Client Approved' and WDM.clientApproval = 1)) "
 * + "GROUP BY PCM.id") public List<Object[]> getPlantActualHrs(@Param("projId")
 ...
 * + "ORDER BY PCM.code") List<PlantMstrEntity>
 * getPlantsForBudget(@Param("clientId") ClientRegEntity clientId,
 * 
 ...
 * ) public ProjectPlantsDtlEntity findBy(@Param("projId") Long
 * projId, @Param("resId") Long resId);
 * 
 * @Modifying
 * 
 ...
 * ) public void updateApproverDetailsByIds( @Param("plantIds") List<Long>
 * plantIds, @Param("approverUserId") Long
 * approverUserId, @Param("originatorUserId") Long
 * originatorUserId, @Param("itemStatus") String itemStatus );
 * 
 * @Modifying
 * 
 * @Query("UPDATE ProjectPlantsDtlEntity PJP SET PJP.itemStatus=:itemStatus,PJP.status=:status WHERE PJP.id in :plantIds"
 * ) public void updateApprovalStatusByIds( @Param("plantIds") List<Long>
 * plantIds, @Param("itemStatus") String itemStatus, @Param("status") Integer
 * status );
 * 
 * @Modifying
 * 
 ...
 * ) public void updateReturnItemDetailsById( @Param("plantId") Long
 * plantId, @Param("returnedByUserId") Long returnedByUserId
 * , @Param("isItemReturned") Integer isItemReturned, @Param("comments") String
 * comments, @Param("itemStatus") String itemStatus );
 * 
 * @Modifying
 * 
 * @Query("UPDATE ProjectPlantsDtlEntity PJP SET PJP.itemStatus=:itemStatus WHERE PJP.id=:plantId"
 * ) public void updatePlantItemStatusById( @Param("plantId") Long
 * plantId, @Param("itemStatus") String itemStatus ); }
 */