...
public interface PrecontractRepository extends ProcurementBaseRepository<PreContractEntity, Long> {
    ...
    @Query("SELECT PRC FROM PreContractEntity PRC  WHERE PRC.projId.projectId =:projId  AND  PRC.preContarctStatus=:preContarctStatus "
    ...
    @Query("SELECT PRC FROM PreContractEntity PRC  WHERE PRC.id=:contractId  AND PRC.projId.projectId =:projId  AND  PRC.status=:status ORDER BY PRC.updatedOn DESC")
    ...
    @Query("SELECT PRC FROM PreContractEntity PRC  WHERE PRC.id in (:contractIds)  AND PRC.projId.projectId in (:projIds)  AND  PRC.status=1 ORDER BY PRC.updatedOn DESC")
    ...
    @Query("SELECT DISTINCT PRC FROM PreContractEntity PRC JOIN PRC.preContractsCmpEntities CMP WHERE PRC.status=:status AND "
    ...
    @Modifying
    @Query("UPDATE PreContractEntity T SET T.contarctStageStatus=:contarctStageStatus  WHERE T.id=:preContractId")
    ...
    @Modifying
    @Query("UPDATE PreContractEntity P  SET  P.status=:status WHERE  P.id in :contractIds")
