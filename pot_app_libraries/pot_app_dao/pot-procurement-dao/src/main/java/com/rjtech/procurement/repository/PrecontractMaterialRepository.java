...
public interface PrecontractMaterialRepository extends ProcurementBaseRepository<PreContractsMaterialDtlEntity, Long> {
    ...
    @Query("SELECT PCM FROM PreContractsMaterialDtlEntity PCM  WHERE PCM.preContractEntity.id=:contractId AND  PCM.status=:status ORDER BY PCM.id")
    List<PreContractsMaterialDtlEntity> findPreContractMaterials(@Param("contractId") Long contractId,
            @Param("status") Integer status);

    @Query("SELECT PCM FROM PreContractsMaterialDtlEntity PCM JOIN FETCH "
    ...
    @Modifying
    @Query("UPDATE PreContractsMaterialDtlEntity PRC SET PRC.status=:status  WHERE PRC.id in :materialIds")
