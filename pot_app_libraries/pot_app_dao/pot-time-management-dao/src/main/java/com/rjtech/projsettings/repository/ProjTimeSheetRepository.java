...
public interface ProjTimeSheetRepository extends JpaRepository<TimeSheetAdditionalTimeEntity, Long> {
    ...
    @Query("SELECT T FROM TimeSheetAdditionalTimeEntity T where T.projId=:projId and T.projCrewMstrEntity.id=:crewId "
    ...
    @Query("SELECT T FROM TimeSheetAdditionalTimeEntity T where T.projId=:projId and T.projCrewMstrEntity.id is null "
    ...
    @Query("SELECT T FROM TimeSheetAdditionalTimeEntity T where T.projId=:projId  "
