...
public interface ProjCrewRepositoryCopy extends JpaRepository<ProjCrewMstrEntity, Long> {
	
	@Query("SELECT CRW FROM ProjCrewMstrEntity CRW  WHERE  CRW.projId.projectId =:projectId AND CRW.desc=:crewCode")
    ProjCrewMstrEntity findCrewByCrewCodeAndProjId(@Param("projectId") long projectId,
