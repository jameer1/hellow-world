...
public interface ResourceAssignmentDataRepository extends JpaRepository<ResourceAssignmentDataEntity, Long> {
	...
	@Query("SELECT TR FROM ResourceAssignmentDataEntity TR "
			+ "WHERE TR.scheduleActivityDataSetEntity.projMstrEntity.id IN :projectIds "
			+ "AND TR.scheduleActivityDataSetEntity.type = 'R' "
			+ "AND TR.scheduleActivityDataSetEntity.baseline = 'Y' ")
	public List<ResourceAssignmentDataEntity> findAllBaselines(@Param("projectIds") List<Long> projectIds);
